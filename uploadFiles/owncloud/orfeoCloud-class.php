...
//put your code here
    ...
    function ListarImagenes($ruta_owonclod, $usA, $carpS, $user, $listaCombo) {
        ...
        for ($index = 0; $index < count($usA); $index++) {
            ...
            exec("ls -lh --full-time " . $ruta_owonclod . "/" . $usA[$index] . "/files/$carpS/ | awk '{ print $9,$5,$6,$7 }'", $data);
            if (count($data) > 0)
			...
			$jsfuntion = "subir2('div$name','$name',0,'" . $usA[$index] . "','$tamano','tp$name')";
                        ...
                        $jsfuntion = "subir('div$name','$name',0,'" . $usA[$index] . "','$tamano')";
                    }
                    $resp.="<tr data-id='$i' data-file='$name' data-type='file' data-mime='application/pdf' style='$styyle' data-size='$size' >
				<td class='filename svg ui-draggable ui-droppable'><img src='/owncloud/core/img/filetypes/application-pdf.png'>					
	    ...
	    //var_dump( $comando);
            ...
            if ($data) {
                ...
                for ($i = 0; $i < count($data); $i++) {
                    ...
                    $daddd = explode('.', $g[0]);
                    // print_r($daddd);
                    $radi = substr($daddd[0], 0, 14);
                    $radi2[$i] = substr($g[0], 0, 14);
                    $name[$i] = $g[0];
                ...
                for ($pp = 0; $pp < $i; $pp++) {
                    ...
                    if ($ext[$pp] == 'pdf' or $ext[$pp] == 'PDF' or $ext[$pp] == 'TIF' or $ext[$pp] == 'tif') {
                        //print_r($dataRadIma);
...
//quito los decimales a los días de diferencia 
    ...
    function subir($rutaArch, $arch, $dependencia, $codusuario, $id_rol, $pages) {
        $rad = substr($arch, 0, 14);
        //echo "** $rutaArch - $rad - $arch, $id_rol -  $codusuario - $dependencia***";
        /*         * * validacion ** */
        //if ($this->validar($rad) == 'no') {
            $this->modelo->actualizar($rutaArch, $rad, $arch, $id_rol, $codusuario, $dependencia, $pages, 'Asociar Imagen a Radicado');
    ...
    //CARLOS RICAURTE 20/06/2019
    //OPCION PARA MODIFICAR IMAGENES historico
    function subirC($rutaArch, $arch, $dependencia, $codusuario, $id_rol, $pages,$obs) {
        $rad = substr($arch, 0, 14);
            $this->modelo->actualizar($rutaArch, $rad, $arch, $id_rol, $codusuario, $dependencia, $pages, $obs);
            return "Actualizado con exito";
    }
    ...
    function subirf($rutaArch, $arch, $dependencia, $codusuario, $id_rol, $pages) {
        ...
        /*         * * validacion ** */
        if ($this->validar($rad) == 'no') {
            $this->modelo->actualizarf($rutaArch, $rad, $arch, $id_rol, $codusuario, $dependencia, $pages, 'Asociar Imagen a Radicado');
            return "Actualizado con exito";
        } else {
            return "Elradicado ya tiene imagen";
        }
    ...
    function subirA($rutaArch, $arch, $dependencia, $codusuario, $id_rol, $peso, $tpdoc, $comentario) {
        //        echo "subirA($rutaArch,$arch,$dependencia,$codusuario,$id_rol,$pages,$peso)";
        $radi = substr($arch, 0, 14);
        //validar radicado
//echo "<hr>";
        echo $validar = $this->modelo->validarRad($radi);
        $auxnumero = '';
        $anex = '';
        if ($validar == 1) {
		...
		return $anex;
           ...
           $ext = strtolower($ext);
            ...
            $anex = $this->modelo->grabarAnexo($radi, $anex_codi, $codigoExtension, $peso, $auxsololect, $_SESSION['krd'], $anexDesc, $auxnumero, $archivoconversion, $dependencia, $rutaArch, $arch, 0);
            $validarAnex = $this->modelo->validarAnex($anex);
            if ($validarAnex == 1)
                return $anex;
            else {
                return 'No se puede anexar';
            }
        } else {
            return 'No es un número valido de radicado';
        }
