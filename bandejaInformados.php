...
$db->conn->SetFetchMode(ADODB_FETCH_ASSOC);
  ...
  */
  ...
  $sqlNoRad = "
               ...
               from
                    radicado b left  join SGD_TPR_TPDCUMENTO c on
                    b.tdoc_codi=c.sgd_tpr_codigo left join SGD_DIR_DRECCIONES d on
...
*/
          ...
          $fechaHoy = date("Y-m-d");
                 ...
                 //echo "--"."<pre>$isql</pre>"; exit;
                  ...
                  while(!$rs->EOF){
                    ...
                    $radiLeido             = $rs->fields["HID_RADI_LEIDO"];
          					...
          					if($depe_actu != "999"){
          							if(!empty( $fechVec)){
          								...
          								//$dias = $calendario->calcularDias($fechVec, date('Y-m-d'));
                           $dias = $vDias->diasHabiles($fechaHoy,$fechVec);
          							}else{
          								$dias = " Sin Clasificar";
          							}
          					}else{
					...
					$resulVali  = $verLinkArchivo->valPermisoRadi ($numeroRadicado);
					$valImg     = $resulVali ['verImg'];
					$pathImagen = $resulVali ['pathImagen'];
					$fldsSGD_EXP_SUBEXPEDIENTE = $resulVali ['numExpe'];
					/////
					//../verradicado.php?verrad=$fldRADI_NUME_RADI&" . session_name () . "=" . session_id () . "&carpeta=9&nomcarpeta=Busquedas&tipo_carp=0'
                    //$linkVerRadicado = "./verradicado.php?verrad=$numeroRadicado&nomcarpeta=$nomcarpeta";
                    $linkVerRadicado = "./verradicado.php?verrad=$numeroRadicado&nomcarpeta=$nomcarpeta&adjuntaInformado=1";
 ...
 if(empty($radiPath)){
     echo "<td class=\"inbox-data-from\">
      ...
      echo "<td><small style=\"padding: 5px;\"><a $anexEstado_linkradi  $anexEstadoEstiloLink href='javascript:void(0)' onclick=\"funlinkArchivo('$numeroRadicado','$ruta_raiz');\"> $numeroRadicado</a></small></td>";
 }
?>
		      <td class="inbox-data-from"> <div><small><a href='<?=$linkVerRadicado?>' target="mainFrame"><?=$fechaRadicado?></a></small></div></td>
                      ...
                      <td class="inbox-data-from"> <div> <small><?=$dias?></small> </div> </td>
