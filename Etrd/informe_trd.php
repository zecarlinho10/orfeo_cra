...
<form name="inf_trd"  action='../Etrd/informe_trd.php?<?=session_name()."=".session_id()."&krd=$krd&fecha_h=$fechah"?>' method=post>
<TABLE width="550" class='borde_tab' align="center">
  ...
  <TR>
	...
	$sqlD = "select $sqlConcat ,depe_codi from dependencia where dependencia_estado=1
							order by depe_codi";
...
//parametro solo de la SSPD
	...
	$order_isql = " and m.depe_codi not in (999,910,905,900,830,820,810,800,700,600,500,400,380,340,321,300,210,200,110,120,130,140,100,1,410,420,430) order by m.depe_codi, m.sgd_srd_codigo,m.sgd_sbrd_codigo,m.sgd_tpr_codigo ";
...
<td align="center" rowspan="2">Series, Subseries y Tipos Documentales</td>
...
<?
	while(!$rs->EOF and $rs)
	{
			...
			$nTDoc = lcfirst($rs->fields['SGD_TPR_DESCRIP']);	//Nombre Tipo Documental
			if($nSRD==$nSRD_ant)
			{
				$pSRD=""; 
			}else{
