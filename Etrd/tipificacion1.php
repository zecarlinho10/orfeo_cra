...
//$db->conn->debug = true;
    ...
    if (isset($_POST['actuaciones'])){$actuaciones = $_POST['actuaciones']; }
...
<tr >
	...
	$querySerie = "select distinct ($sqlConcat) as detalle, s.sgd_srd_codigo 
			 ...
			 where (m.depe_codi = '$codepe' or ".$funciones->instr("DEPE_CODI_APLICA","'$codepe'")." > 0 ) 
...
?> 	
    ...
    <tr>
		<td>
			<input type="checkbox" name="actuaciones" value="1"> Actuaciones Administrativas<br>
		</td>
	</tr>
    </table>
<br>
...
if ($insertar_registro && $tdoc !=0 && $tsub !=-1 && $codserie !=0 )
{
	...
	//$db->conn->debug = true;
		...
		$isqlTRD = "select SGD_MRD_CODIGO 
					...
					where (depe_codi = '$codepe' or ".$funciones->instr("DEPE_CODI_APLICA","'$codepe'")." > 0 ) 
			...
			$radiModi = $Historico->insertarHistorico($codiRegH, $dependencia, $codusuario, $dependencia, $codusuario, $observa, 32); 
		  	...
		  	*/
		  	/*	CARLOS RICAURTE 1/8/2018
					ACTUACION AL MODIFICAR TRD TOMA LA FECHA DE HOY PARA CALCULAR FECHA DE VENCIMIENTO
		  		*/
				$fecha_real="";
		  		if($actuaciones==1){

		  			//$isqlA = "SELECT RADI_FECH_RADI FROM RADICADO WHERE RADI_NUME_RADI = '$nurad'";
		  			$isqlA="SELECT " . $db->conn->SQLDate('Y-m-d H:i:s','RADI_FECH_RADI') ." AS FECH_RADI FROM RADICADO WHERE RADI_NUME_RADI = '$nurad'";
		  			
					$rsA=$db->query($isqlA);

					while(!$rsA->EOF){
					    $fecha_real = $rsA->fields["FECH_RADI"]; 
						$rsA->MoveNext();
					}
					
		  			$hoy= Date("Y-m-d");

		  			$sql="UPDATE RADICADO SET RADI_FECH_RADI = '$hoy' WHERE RADI_NUME_RADI='$nurad'";
		  			$db->query($sql);
		  		}
		 		$radiUp = $trd->actualizarTRD($codiRegH,$tdoc);
		 		//CARLOS RICAURTE 15/02/2019
		 		//PONE BANDEJA DE TRASLADOS
		 		if($tdoc==1987){
		 			$sql="UPDATE RADICADO SET CARP_CODI= 13, CARP_PER=0 WHERE RADI_NUME_RADI='$nurad'";
					$db->conn->Execute($sql);
		 		}
		 		//VUELVE A DEJAR LA FECHA ORIGINAL
		 		
		 		if($actuaciones==1){
		 			$sql="UPDATE RADICADO SET RADI_FECH_RADI = TO_DATE('$fecha_real', 'yyyy/mm/dd hh24:mi:ss')  WHERE RADI_NUME_RADI='$nurad'";
					$db->conn->Execute($sql);
		  		}
  		...
  		if ($modificar && $tdoc !=0 && $tsub !=-1 && $codserie !=0 ){
				$sqlH = "SELECT $radi_nume_radi RADI_NUME_RADI,
				        SGD_MRD_CODIGO 
						FROM SGD_RDF_RETDOCF r
						WHERE RADI_NUME_RADI = '$nurad'";
				...
				$TRD = $codiActu;
		        include "$ruta_raiz/radicacion/detalle_clasificacionTRD.php";
				// David Amaya Beltran - agrega la descripcion de la modificacion de la TRD
				$desc=$_POST['desc'];
...
and radi_depe_radi not in(999,900)
   		...
   		while(!$rsC1->EOF)
			{
				...
				$linkVerRadicado = "../verradicado.php?verrad=$dserie1&nomcarpeta=$nomcarpeta#tabs-d";
		?> 
		
				<td class="listado2">
					<small><a href="<?=$linkVerRadicado?>" target="_blank"><?=$dserie1?></a></small>
				</td>
