...
if(!$tdoc) $tdoc = 0;
	$isql = "select a.SGD_FLD_CODIGO,
		 ...
		 ,a.SGD_FLD_IMAGEN
		 ,a.SGD_FLD_GRUPOWEB
		 FROM  SGD_TPR_TPDCUMENTO B left join  SGD_FLD_FLUJODOC a
  		   ON a.sgd_tpr_codigo=b.SGD_TPR_CODIGO
		 where
...
<tr>
	<td colspan="20" class="titulos2">ESTADO DEL DOCUMENTO </td>
  </tr>
<?
...
<td >
<?
$fechaFinal = date('Y-m-d') - $fechaVencimiento;
if(!$tLegal) $tLegal = $diasPlazo;
if($tLegal>=1)	
{
 echo "".$tLegal;
?> Dias
...
<table width="100%" border="0">
  <tr>
    ...
    <th scope="col"><table width="90%" class="borde_tab" align="center" >
      <tr>
        <td height="25"  align="center"><div><a href='#' class='hideURL' onclick="Start('./verHistorico.php?verrad=<?=$verrad?>&cmp=<?=$password?>&fechah=<?=$fechah?>',300,400);"> <span class=titulos2>VER HISTORICO DEL DOCUMENTO</span></a></div></td>
      </tr>
    </table></th>
  </tr>
</table>
