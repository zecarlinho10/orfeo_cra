...
$isql="SELECT S.DEPE_CODI, D.DEPE_NOMB, S.SGD_SRD_CODIGO, SE.SGD_SRD_DESCRIP, S.SGD_SBRD_CODIGO, SS.SGD_SBRD_DESCRIP, SGD_SEXP_PAREXP1, SGD_SEXP_PAREXP2, SGD_SEXP_PAREXP3, SGD_SEXP_PAREXP4,S.SGD_EXP_NUMERO
        ...
        WHERE E.SGD_EXP_NUMERO = S.SGD_EXP_NUMERO AND S.DEPE_CODI=D.DEPE_CODI AND SE.SGD_SRD_CODIGO=S.SGD_SRD_CODIGO AND 
              S.SGD_SBRD_CODIGO=SS.SGD_SBRD_CODIGO AND S.SGD_SRD_CODIGO=SS.SGD_SRD_CODIGO AND E.SGD_EXP_ESTADO <> 2 AND 
...
while (!empty($rs) && !$rs->EOF) {
    ...
    else if($rs->fields["SGD_SEXP_PAREXP2"]!=NULL){
        $nomexp=$rs->fields["SGD_SEXP_PAREXP2"];
    }
    else if($rs->fields["SGD_SEXP_PAREXP3"]!=NULL){
        $nomexp=$rs->fields["SGD_SEXP_PAREXP3"];
    }
    else{
        $nomexp=$rs->fields["SGD_SEXP_PAREXP4"];
    }
