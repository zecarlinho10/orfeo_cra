 ...
 * @mail    cricaurte.arsenal@gmail.com   
 * @version     1.1
 */
$ruta_raiz = "../";
session_start();
...
function pintarResultados($fila,$i,$n){
		...
		switch($n){
			case 0:
				if($fila['PATH']!=NULL){
					$radi = $fila['RADI_NUME_RADI'];
					$path = $fila['PATH'];
						$salida = "<a href='javascript:void(0)' onclick=\"funlinkArchivoHistorico('$path','$ruta_raiz');\"> $radi</a>";
				}
				else
...
<script type="text/javascript">
    ...
    function funlinkArchivoHistorico(path, rutaRaiz){
        var nombreventana = "linkVistArch";
        var url           = "linkArchivoHistorico.php?<? echo session_name()."=".session_id()?>"+"&path="+path;
