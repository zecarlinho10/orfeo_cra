                  ...
                  <table class="table table-bordered table-striped">
                      ...
                      <tr>
                        ...
                        <td class="listado5">
                          ...
                          -->
                          <input class="tex_area" type="text" name="s_DOCTO" maxlength="" value="<?=tohtml($ps_DOCTO) ?>" size="" >
                        </td>
                      </tr>
             ...
             <input class="tex_area" type="text" name="s_CUENTAINTERNA" maxlength="" value="<?=tohtml
                  ...
                  ?>
                    ...
                    if(is_array($lookup_s_entrada))
                    {
                      ...
                      foreach($lookup_s_entrada as $key => $value) // while(list($key, $value) = each($lookup_s_entrada))
                      {
                    ...
                    $lookup_s_TDOC_CODI = db_fill_array("SELECT DISTINCT T.SGD_TPR_CODIGO, SGD_TPR_DESCRIP || ' - ' ||
                                                        CASE T.SGD_TPR_ESTADO 
                                                         WHEN 0 THEN 'Inactiva'
                                                         ELSE 'Activa' 
                                                        END
                                                        FROM SGD_TPR_TPDCUMENTO T, SGD_MRD_MATRIRD M
                                                        WHERE T.SGD_TPR_CODIGO = M.SGD_TPR_CODIGO
                                                        ORDER BY 2");

                    if(is_array($lookup_s_TDOC_CODI))
                    {
                      reset($lookup_s_TDOC_CODI);
                      foreach($lookup_s_TDOC_CODI as $key => $value) // while(list($key, $value) = each($lookup_s_TDOC_CODI))
                      {
                  ...
                  if(is_array($lookup_s_RADI_DEPE_ACTU))
                  {
                    ...
                    foreach($lookup_s_RADI_DEPE_ACTU as $key => $value) //while(list($key, $value) = each($lookup_s_RADI_DEPE_ACTU))
                    {
...
<!--                <td class="titulos5"><a class="vinculos" href="<?=$sFileName?>?<?=$form_params?>FormCIUDADANO_Sorting=23&FormCIUDADANO_Sorted=<?=$form_sorting?>&">Dignatario</a></td>-->
               <!--<td class="titulos5"><a class="vinculos" href="<?=$sFileName?>?<?=$form_params?>FormCIUDADANO_Sorting=4 &FormCIUDADANO_Sorted=<?=$form_sorting?>&">Cuenta Interna</a></td>-->
                ...
                <td class="titulos5"><a class="vinculos" href="<?=$sFileName?>?<?=$form_params?>FormCIUDADANO_Sorting=5&FormCIUDADANO_Sorted=<?=$form_sorting?>&">Numero de Paginas</a></td>
                ...
                <td class="titulos5"><a class="vinculos" href="<?=$sFileName?>?<?=$form_params?>FormCIUDADANO_Sorting=21&FormCIUDADANO_Sorted=<?=$form_sorting?>&">Dias Hábiles</a></td>
              </tr>
...
/*
  $sqlsum="SELECT U.ID, M.AUTG_ID FROM USUARIO U, AUTM_MEMBRESIAS M
  WHERE M.AUTG_ID=4";
  $rs1 = $db->conn->Execute($sqlsum);
      
    if(($rs1->fields['AUTG_ID'])!=4){
        $sWhere.=" and substr(r.radi_nume_radi,-1)!=4";
    }
*/
  
  $codusuarioz=$_SESSION["codusuario"];
  $dependenciaz = $_SESSION["dependencia"];

$sqlsum="SELECT U.ID, G.DESCRIPCION
FROM USUARIO U, AUTM_MEMBRESIAS M, AUTG_GRUPOS G
WHERE U.USUA_CODI='$codusuarioz' AND U.DEPE_CODI='$dependenciaz' AND M.AUTU_ID=U.ID AND AUTG_ID=4 AND G.ID=M.AUTG_ID";

$rs1 = $db->conn->Execute($sqlsum);

if(empty($rs1->fields['DESCRIPCION'])){
    $sWhere.=" and substr(r.radi_nume_radi,-1)!=4";
}
...
if ($_SESSION["nivelus"]==5 or $seguridadRadicado == 0 ){
$linkDocto = "<a class=\"vinculos\" href=\"#2\" onclick=\"funlinkArchivo('$fldRADI_NUME_RADI','$ruta_raiz');\" target\"Imagen$iii\">";
$linkInfGeneral = "<a class='vinculos' href='../verradicado.php?verrad=$fldRADI_NUME_RADI&" . session_name () . "=" . session_id () . "&carpeta=9&nomcarpeta=Busquedas&tip     o_carp=0'>";
$verImg=true;
}else{
...
$consultaExpediente="SELECT exp.SGD_EXP_NUMERO, sexp.sgd_exp_privado  , u.usua_doc DOC_RESPONSABLE, u.DEPE_CODI DEPE_RESPONSABLE
      ...
      sgd_sexp_secexpedientes sexp left join usuario u
         ...
         WHERE exp.SGD_EXP_ESTADO <> 2 AND 
          exp.radi_nume_radi= $fldRADI_NUME_RADI
          AND exp.sgd_exp_numero=sexp.sgd_exp_numero
          AND exp.sgd_exp_fech=(SELECT MAX(exp2.SGD_EXP_FECH) as minFech from sgd_exp_expediente exp2 where exp2.radi_nume_radi= $fldRADI_NUME_RADI)";
$db->debug = true;
...
<? if ($aRADI_DEPE_ACTU!=999) { 
    //echo $diasrestantes =  $Dha->getDiasRestantes($fldRADI_NUME_RADI,$FECHA_VCMTO,$fldTIPO_DOC);
    //echo $fldRADI_NUME_RADI . "--" . $FECHA_VCMTO . "--" . $fechaHoy = date("Y-m-d") . "--" . $fldTIPO_DOC . "--" . $Dha->getDiasRestantes($fldRADI_NUME_RADI,$FECHA_VCMTO,$fldTIPO_DOC);
  /* 8/5/18 CARLOS RICAURTE
     SE CAMBIA DIAS CALENDARIO POR DIAS HABILES
  */ 
  $fechaHoy = date("Y-m-d");
  echo $diasrestantes = $Dha->diasHabiles($fechaHoy,$FECHA_VCMTO);

  } else {
    echo "Archivado";
  } //echo $diasrestantes =  $Dha->getDiasRestantes($fldRADI_NUME_RADI,$FECHA_VCMTO,$fldTIPO_DOC); // if ($fldRADI_DEPE_ACTU!=999){ echo tohtml($fldDIASR.$drh);} else {echo "Sal";} 
?>&nbsp;
...
function buscar_prueba($nivelus, $tpRemDes, $whereFlds){
    ...
    $titulos=array("#","1#RADICADO","3#FECHA RADICACION","2#EXPEDIENTE","4#ASUNTO","14#TIPO DE DIOCUMENTO","21#TIPO","7#NO DE HOJAS","15#DIRECCION CONTACTO","18#TELEFONO CONTACTO","16#MAIL CONTACTO ","20#DIGNATARIO","17#NOMBRE","19#DOCUMENTO","22#USUARIO ACTUAL","10#DEPENDENCIA ACTUAL","23#USUARIO ANTERIOR","13#DIAS RESTANTES");
