...
function __construct($ruta_raiz){
    ...
    $this->driver = $driver;
	$codificacion =($driver=="oci8po")?"AL32UTF8":"UTF-8";
	$this->conn  = NewADOConnection("$driver");
	//$this->conn->debug=true;
	$this->conn->charSet = $codificacion;
	$ipCliente="".Utils::getIp();
	$this->conn->setUsuaDoc($_SESSION["usua_doc"]);
	$this->conn->setIpClient( $ipCliente);
	//CARLOS RICAURTE 03/07/2019
	$this->conn->setUsuaDoc($_SESSION["usua_doc"]);
...
function sysdate()
{
  ...
  if($this->driver == "oci8" ||  $this->driver == "oci8po")  return "sysdate";
...
function limit($numRows)
{
  ...
  if($this->driver == "oci8" || $this->driver == "oci8po" )  $this->limitOci8 = " and ROWNUM <= $numRows";
