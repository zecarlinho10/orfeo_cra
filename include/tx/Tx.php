<?php
include_once(realpath(dirname(__FILE__) . "/")."/Historico.php");
...
//ini_set("display_errors",1);	
	...
	if($infConjunto>=1) $codTxNew = 66; else $codTxNew = 8;
        $this->insertarHistorico($tmp_rad,$depOrigen,$codUsOrigen,$depDestino,$codUsDestino, $observa,$codTxNew);
	}
}
...
function borrarInformado( $radicados, $loginOrigen,$depDestino,$depOrigen,$codUsDestino, $codUsOrigen,$observa){
  ...
  //*****David*****
  function abrir_exp($expedientes, $loginOrigen ,$depOrigen ,$codUsOrigen,$observa)
  {
  	$systemDate = $db->conn->sysTimeStamp;
  	$expedientesIn = join(",",$expedientes);
  	$sql="update sgd_sexp_secexpedientes
  		  set SGD_CERRADO=NULL
  		  where SGD_EXP_NUMERO IN ($expedientesIn)";
  	$this->db->conn->debug=false;
  	$rs = $this->db->conn->Execute($sql);
  	$this->insertarHistoricoExp($expedientes,$radicados,$depOrigen,$codUsOrigen, $observa, 59,0);
  	return $sql;
...
function changeFolder( $radicados, $usuaLogin,$carpetaDestino,$carpetaTipo,$tomarNivel,$observa)
  ...
  function reasignar( $radicados, $loginOrigen,$depDestino,$depOrigen,$codUsDestino, $codUsOrigen,$tomarNivel, $observa,$codTx,$carp_codi)
  {
   ...
   $this->db->conn->SetFetchMode(ADODB_FETCH_ASSOC);
	...
	//carlos ricaurte ejecuta carpetas traslados
	$isqlT=$this->generaQueryBandejaTraslados($radicadosIn);
	if($isqlT!=""){
		$this->db->conn->Execute($isqlT); # Ejecuta para dejar en bandeja los traslados
	}
  ...
  //carlos ricaurte 12/2/2019
  //funcion genera query para dejar los traslados TDOC_CODI = 1987 en la bandeja traslados

  function generaQueryBandejaTraslados($radicadosIn){
  	
	//$this->db->conn->debug=true;
	$sqlConfirmaTraslado="select RADI_NUME_RADI from RADICADO where TDOC_CODI = 1987 and RADI_NUME_RADI in($radicadosIn)";
		
	$rs1 = $this->db->query($sqlConfirmaTraslado);
        $vecRadicadosTrasladados = array();
	while (!empty($rs1) && !$rs1->EOF) {
		$radicadosTraslado=$rs1->fields["RADI_NUME_RADI"];
		$vecRadicadosTrasladados[]=$radicadosTraslado;
		$rs1->MoveNext();
	}
	if(count($vecRadicadosTrasladados) > 0 ){
		$radicadosTrasladados = join(",",$vecRadicadosTrasladados);
		$isqlT = "update radicado
				set
				  CARP_CODI=13
				  ,CARP_PER=0
			 where RADI_NUME_RADI in($radicadosTrasladados)";
		return $isqlT;
	}else{
		return "";
	}
  ...
  function devolver( $radicados, $loginOrigen,$depOrigen, $codUsOrigen,$tomarNivel, $observa)
  {
	...
	//echo "<hr> $codUsDestino <hr>";
		...
		if($codUsDestino)
		{
			...
			$radicadosIn = join(",",$radicados);
			//13/02/2019 carlos ricaurte diferencia traslados
			$proccarp= "Dev. ";
			$carp_codi= 12;
			$carp_per = 0;
			$isql = "update radicado
			...
			$isqlT=$this->generaQueryBandejaTraslados($radicadosIn);
			if($isqlT != ""){
				$this->db->conn->Execute($isqlT); # Ejecuta la query de traslados
			}
			$rad[]=$noRadicado;
