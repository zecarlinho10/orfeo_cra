...
class Expediente
{
    ...
    var $num_expediente;
 // Almacena el nume del expediente
    var $estado_expediente;
 // Almacena el estado 0 para organizacion y 1 para indicar ke ya esta clasificado fisicamente en archivo
    var $descSerie;
     ...
     *
     * @param $codigoTipoExp int
     *            Codigo del tipo de expediente o Tipo de Proceso.
     */
    var $codigoTipoExp;
     ...
     * Variable que ALmacena los dias Habiles de un Proceso
     *
     * @param $expTerminos int
     *            Contiene los dias Habiles de Un Proceso.
     */
    var $expTerminos;
     ...
     *
     * @param $expTerminosP int
     *            Contiene los terminos en dias Habiles de la Etapa Actual del Proceso del expediente.
     */
    var $expTerminosP;
     ...
     * Variable
     *
     * @param $expFechaCrea int
     *            Almacena Fecha de Creacion del expediente
     */
    // var $expTerminos;
    var $expFechaCrea;
     ...
     * Variable
     *
     * @param $numSubexpediente int
     *            Almacena el numero de subexpediente;
     */
     ...
     * Variable
     *
     * @param $pAutomatico int
     *            Dice si el expediente se puede manejar de forma manual (Los cambios de estado);
     */
    var $pAutomatico;
     ...
     *
     * @param $db variable
     *            contenedora del Cursor. Esta tiene que ser enviada en el construtor
     */
     ...
     *
     * @param $radicado int
     *            Contiene el numero de radicado a Buscar
     *            
     ...
     * @param $radicado int
     *            Contiene el numero de radicado a Buscar
     * @param $usua_doc String
     *            Documento de identificacion de Usuario que realiza la insercion-
     * @param $usua_codi String
     *            Codigo en Orfeo del Usuario que realiza la insercion-
     * @param $depe_codi String
     *            Codigo en Orfeo de la dependencia del Usuario que realiza la insercion.
     * @param $radicado Numeric
     *            Numero de Radicado a Relacionar con el Expediente.
     * @param $radicado String
     *            Numero de Expediente a Relacionar con el Radicado.
     * @param $expManual String
     *            que Contien 1 o 0. "0 es genera secuencia y 1 deja el consecutivo que el usuario ha colocado.
     *            
     * @return Numero de Expediente que posee el radicado
     *        
     */
    function insertar_expediente($num_expediente, $radicado, $depe_codi, $usua_codi, $usua_doc)
    {
        ...
        if (! $rs->EOF or $creadoOld == 1) {
            ...
            // 10/8/18
            // CARLOS RICAURTE
            // INGRESAR HIJOS EN EXPEDIENTES
            $sqlHijos = "SELECT RADI_NUME_RADI FROM RADICADO WHERE RADI_NUME_DERI='$radicado'";
            $rsHijos = $this->db->conn->query($sqlHijos);
            $nradicado=$rsHijos->fields["RADI_NUME_RADI"];
            while (!$rsHijos->EOF AND !empty($rsHijos)) {
                $query = "insert into SGD_EXP_EXPEDIENTE(SGD_EXP_NUMERO   , RADI_NUME_RADI,SGD_EXP_FECH,DEPE_CODI   ,USUA_CODI   ,USUA_DOC      ,SGD_EXP_ESTADO )
                VALUES ('$num_expediente','$nradicado'    ," . $sqlFechaHoy . ",'$depe_codi' ,'$usua_codi' ,'$usua_doc','$estado_expediente')";
                if (! $this->db->conn->query($query)) {
                    // echo '<br>Lo siento no pudo agregar el expediente<br>';
                    ...
                    $q_excluido .= " FROM SGD_EXP_EXPEDIENTE";
                    $q_excluido .= " WHERE SGD_EXP_NUMERO = '" . $num_expediente . "'";
                    $q_excluido .= " AND RADI_NUME_RADI = '" . $nradicado . "'";
                    $rs_excluido = $this->db->conn->query($q_excluido);
                    if ($rs_excluido->fields["SGD_EXP_ESTADO"] == 2) {
                        $q_update = "UPDATE SGD_EXP_EXPEDIENTE";
                        $q_update .= " SET SGD_EXP_ESTADO = 0";
                        $q_update .= " WHERE SGD_EXP_NUMERO = '" . $num_expediente . "'";
                        $q_update .= " AND RADI_NUME_RADI = '" . $nradicado . "'";
                        $q_update .= " AND SGD_EXP_ESTADO = 2";
                        if (! $rs_update = $this->db->conn->query($q_update)) {
                            $retorno= 0;
                        } 
     ...
     * Crea un expediente y anade un numero de radicado a este.
     * Ademas inserta en el historico
     ...
     *            Numero de Radicado a Relacionar con el Expediente.
     * @param $expOld String
     *            Si esta en False Indica eque es un expediente Normal, True es del numeracin Antigua.
     * @param $radicado String
     *            Numero de Expediente a Relacionar con el Radicado.
     *            
     * @return Numero de Expediente que posee el radicado
     ...
     *        
     * @param $codiPROC Numeric
     *            Codigo del proceso asociado al expediente
     * @param $arrParametro Array
     *            Arreglo que contiene los parametros asociados al expediente
    ...
    function crearExpediente($numExpediente, $radicado, $depe_codi, $usua_codi, $usua_doc, $usuaDocExp, $codiSRD, $codiSBRD, $expOld = null, $fechaExp = null, $codiPROC = null, $arrParametro = null)
    {
        ...
        if ($expOld == "false") {
            ...
            if (strlen($codiSRD) < 2) {
                $trdExp = substr("00" . $codiSRD, - 2);
            } else {
                $trdExp = $codiSRD;
            }
            if (strlen($codiSBRD) < 2) {
                $trdExp .= substr("00" . $codiSBRD, - 2);
            } else {
                $trdExp .= $codiSBRD;
            }
     ...
     * y le incrementa 1.
     *
     * @param $dependencia int
     *            Codigo de la Dependencia.
     * @param $codiSrd int
     *            Codigo de la Serie documental que es enviada por el Usuario.
     * @param $codiSBRD int
     *            Codigo de la subserie documental enviada por el Usuario.
     * @param $query String
     *            Cadena de uso temporal para guardar consultas SQL.
     *            
     ...
     * Modifica un expediente y aÃ±ade un numero de radicado a este.
     * Ademas inserta en el historico
     * La transcacion realizada, para esto verifica que el digito de Chequeo.
     *
     * @param $radicado int
    ...
    function consultaTipoExpediente($numExpediente)
    {
        ...
        if ($numExpediente) {
            ...
            IF ($this->codigoFldExp != 0) {
                ...
                $cuentaEtapas = (! empty($rs)) ? $rs->FieldCount() : 0;
     ...
     * @param $codiSbrd int
     *            Codigo de la subserie documental.
     * @param $codiProc int
     *            Codigo del proceso.
     *            Retorna:
     *            
     ...
     *
     * @param $numExpediente String
     *            Numero de Expediente a buscar.
     *            
    ...
    /**
     * FUNCION confirma dependencia
     * Determina si el usuario que inserta radicados pertenece a la misma dependencia.
     *
     * @param $numExpediente String Numero de Expediente a buscar.
              $dependencia usuario Numero dedependencia del usuario.
     *            
     * @return 0 si no pertenece a la misma dependencia.
     *         Fecha de creacion: 04-02-2019
     *         Creador: Carlos Ricaurte
     *         Fecha de modificacion:
     *         Modificador:
     */
    function confirmaDependencia($numExpediente,$krd)
    {
        $dep_exp=0;
        $id=0;
        $dependencia=0;
        ...
        /*OBTENER DATOS DEL USUARIO A PARTIR DE USUA_LOGIN*/
        $queryU = "SELECT USUA_CODI, DEPE_CODI, ID";
        $queryU .= " FROM USUARIO";
        $queryU .= " WHERE USUA_LOGIN ='" . $krd ."'";
        $rs =  $this->db->conn->query($queryU);
        while ($rs && ! $rs->EOF) {
            $id         = $rs->fields['ID'];
            $dependencia= $rs->fields['DEPE_CODI'];
            $codigo     = $rs->fields['USUA_CODI'];
            $rs->MoveNext();
        }

        /*CONFIGURA EL WHERE PARA QUE ACEPTE EXPEDIENTES DE CODIGOS VIEJOS*/
        $and="' AND DEPE_CODI = " . $dependencia;
        if($dependencia==12 || $dependencia==216){
            $and="' AND ( DEPE_CODI = 12 OR DEPE_CODI = 211 OR DEPE_CODI = 216)";
        }
        else if($dependencia==20){
            $and="' AND ( DEPE_CODI = 20 OR DEPE_CODI = 310 )";
        }
        else if($dependencia==30 || $dependencia==402 || $dependencia==403){
            $and="' AND ( DEPE_CODI = 30 )";
        }

        /*VALIDA PERMISO ARCHIVO: AUTG_GRUPOS 2*/
        $queryp = "SELECT COUNT(AUTG_ID) AS TOTAL FROM AUTM_MEMBRESIAS";
        $queryp .= " WHERE AUTG_ID = 2 AND AUTU_ID =" . $id ;
        $rs =  $this->db->conn->query($queryp);
        while ($rs && ! $rs->EOF) {
            $permisoArchivo = $rs->fields['TOTAL'];
            $rs->MoveNext();
        }

        /*VALIDA SI PERTENECE A LAS DEPENDENCIAS DE ARCHIVO*/
        if($dependencia==321 || $dependencia==999 || $permisoArchivo<>0){
            $dep_exp=1;    
        }
        else{
     ...
     *
     * @param $radicado int
     *            Numero del radicado incluido en el expediente a buscar.
     * @param $numExpediente int
     *            Numero del expediente a buscar.
     *            
     ...
     *            Contiene el numero de radicado a Buscar
     * @param $numExpediente String
     *            Numero del Expediente del que se desea excluir el radicado.
     *            
     ...
     *
     * @param $radicado int
     *            Contiene el numero de radicado
     * @param $numExpediente int
     *            Contiene el nï¿½mero del expediente
     * @param $numSubexpediente int
     *            Contiene el nï¿½mero del Subexpediente
     *            
    ...
    public function consultarRadicados($numExpediente, $inicio, $limit)
    {
        $query = "select exp.RADI_NUME_RADI ,r.RA_ASUN, to_char(r.RADI_FECH_RADI, 'YYYY-MM-DD hh:mi:SS') RADI_FECH_RADI, 
			t.SGD_TPR_DESCRIP, r.RADI_PATH  from sgd_exp_expediente exp, radicado r, SGD_TPR_TPDCUMENTO t
			where exp.sgd_exp_numero='" . $numExpediente . "' and exp.radi_nume_radi = r.radi_nume_radi
			 and r.tdoc_codi=t.sgd_tpr_codigo  AND exp.sgd_exp_estado <>2  order by r.radi_fech_radi DESC";
        $rsRad = $this->db->conn->SelectLimit($query, $limit, $inicio);
        $iRr = 0;
        $expArr = array();
        if (! $rsRas->EOF) {
            while (! $rsRad->EOF) {
                ...
                $asuntoRadicado = $rsRad->fields['RA_ASUN'];
                $pathRadicado = $rsRad->fields['RADI_PATH'];
                $expArr[$iRr]["NUM_RADICADO"] = $numRadicado;
                $expArr[$iRr]["FECHA_RADICADO"] = $fechaRadicado;
                $expArr[$iRr]["TIPO_DRADICADO"] = $tipoDRadicado;
                $expArr[$iRr]["ASUNTO_RADICADO"] = $asuntoRadicado;
                $expArr[$iRr]["PATH_RADICADO"] = $pathRadicado;
                $query = "select a.ANEX_CODIGO,a.ANEX_RADI_NUME,a.ANEX_DESC, a.radi_nume_salida, 
                                 a.anex_numero, t.SGD_TPR_DESCRIP, a.ANEX_NOMB_ARCHIVO 
                          from anexos a
                               LEFT JOIN fldoc.SGD_TPR_TPDCUMENTO t ON a.sgd_tpr_codigo=t.sgd_tpr_codigo
						  where a.anex_radi_nume='" . $numRadicado . "'
                ...
                $iA = 0;
                while (! $rsAnex->EOF) {
                    $arrAnexos[$iA]["NUMERO"] = $rsAnex->fields['ANEX_CODIGO'];
                    $arrAnexos[$iA]["DESCRIPCION"] = $rsAnex->fields['ANEX_DESC'];
                    $arrAnexos[$iA]["RADI_SALIDA"] = $rsAnex->fields['RADI_NUME_SALIDA'];
                    $arrAnexos[$iA]["ANEX_NUMERO"] = $rsAnex->fields['ANEX_NUMERO'];
                    $arrAnexos[$iA]["ANEX_PATH"] = $rsAnex->fields['ANEX_NOMB_ARCHIVO'];
                    $expArr[$iRr]["ANEXOS"] = $arrAnexos;
                    $rsAnex->MoveNext();
                    $iA ++;
                }
                $iRr ++;
    ...
    public function totalRadicados($numExpediente)
    {
        $query = "select count(1) TOTAL from 
			SGD_EXP_EXPEDIENTE e, SGD_SEXP_SECEXPEDIENTES sexp
            where e.sgd_exp_numero = '$numExpediente' AND e.sgd_exp_numero=sexp.sgd_exp_numero
			AND SGD_EXP_ESTADO <> 2";
        $rs = $this->db->conn->query($query);
        if (! empty($rs))
    ...
    public function consultaExpedientes($numeroRadicado)
    {
        $query = "select e.SGD_EXP_NUMERO,e.SGD_EXP_ESTADO,e.RADI_NUME_RADI AS RADI_NUME_RADI
		            from SGD_EXP_EXPEDIENTE e, SGD_SEXP_SECEXPEDIENTES sexp
		             where e.RADI_NUME_RADI = $numeroRadicado AND e.sgd_exp_numero=sexp.sgd_exp_numero
					 AND SGD_EXP_ESTADO <> 2";
        $iE = 0;
        ...
        if ($rs == null && $rs->EOF) {
            ...
            while (! $rs->EOF) {
                $estadoExpediente = $rs->fields['SGD_EXP_ESTADO'];
                $numExpediente = $rs->fields['SGD_EXP_NUMERO'];
                if ($iE == 0) {
                    $this->num_expediente = $numExpediente;
                    $this->estado_expediente = $estadoExpediente;
                }
                $expArr[$iE]['numeroExpediente'] = $numExpediente;
                $expArr[$iE]['estadoExpediente'] = $estadoExpediente;
                $iE ++;
                $rs->MoveNext();
            }
        }
    ...
    private function metadataExpediente($numExpediente)
    {
        $isqlDepR = "SELECT u.USUA_NOMB,USUA_DOC_RESPONSABLE, SGD_EXP_PRIVADO,SGD_SEXP_PAREXP1,SGD_SEXP_PAREXP2
		 FROM SGD_SEXP_SECEXPEDIENTES  e left join USUARIO u on e.USUA_DOC_RESPONSABLE=u.USUA_DOC
		 WHERE SGD_EXP_NUMERO = '$numExpediente' ";
        $rsDepR = $this->db->query($isqlDepR);
        $this->seguridad = $rsDepR->fields['SGD_EXP_PRIVADO'];
        $this->expUsuaDoc = $rsDepR->fields['USUA_DOC_RESPONSABLE'];
        $this->param = substr($rsDepR->fields['SGD_SEXP_PAREXP1'], 0, 30) . ' ' . substr($rsDepR->fields['SGD_SEXP_PAREXP2'], 0, 50);
        $this->responsable = $rsDepR->fields['USUA_NOMB'];
    }

    public function consultaEstadoArchivo($numExpediente)
    {
        $sqlb = "select sgd_exp_archivo from sgd_exp_expediente 
		  where sgd_exp_numero = '$num_expediente'";
        $rsb = $this->db->query($sqlb);
        $this->arch = $rsb->fields['SGD_EXP_ARCHIVO'];
    }

    public function getInfoExpediente($numExpediente)
    {
        $this->metadataExpediente($numExpediente);
        $this->consultaEstadoArchivo($numExpediente);
    }

    public function listarOtrosExpedientes($radicado, $expediente)
    {
        $resultado = null;
        $sql = "select e.SGD_EXP_NUMERO,e.SGD_EXP_ESTADO,e.RADI_NUME_RADI AS RADI_NUME_RADI,
		     SGD_SEXP_PAREXP1,SGD_SEXP_PAREXP2
		  from SGD_EXP_EXPEDIENTE e, SGD_SEXP_SECEXPEDIENTES sexp
		  where e.RADI_NUME_RADI = $numeroRadicado AND e.sgd_exp_numero=sexp.sgd_exp_numero
		  AND SGD_EXP_ESTADO <> 2 and  e.sgd_exp_numero<> '$expediente'";
        $rs = $this->db->query($sql);
        $i = 0;
        if ($rs != null && ! $rs->EOF) {
            while (! $rs->EOF) {
                $resultado[$i]["numero"] = $rs->fields["SGD_EXP_NUMERO"];
                $resultado[$i]["param"] = " " . substr($rs->fields['SGD_SEXP_PAREXP1'], 0, 30) . ' ' . substr($rs->fields['SGD_SEXP_PAREXP2'], 0, 50);
                $i ++;
                $rs->MoveNext();
            }
        }
