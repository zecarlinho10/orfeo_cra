...
var $numeroRadicado;
  ...
  /**
   *Variable que contiene el codigo de verificacion para consulta de radicados 
   * @var varchar
   * @access public
   */
...
function idRadicadoConCodigoVerificacion($radicado, $codigoverificacion){
		$q="select 
		 RADI_NUME_RADI
	  from 
		 radicado 
		where RADI_NUME_RADI=$radicado AND SGD_RAD_CODIGOVERIFICACION = '".$codigoverificacion."'";
	$rs=$this->db->query($q);
	$idRad = 0;
	if 	(!$rs->EOF) 
 		{
