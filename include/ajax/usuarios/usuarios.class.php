...
class usuarios{
  ...
  /**
	...
	function getUsuarios($idObjetoHtml,$depeCodi,$txAccion) {
		$response = new HTML_AJAX_Action();
		$indexSelected=0;
	    $iSql = "select (CASE ".$this->db->conn->IfNull('u.SGD_ROL_CODIGO', 0)."  WHEN 0  THEN u.USUA_NOMB WHEN 1 THEN u.USUA_NOMB WHEN 2 THEN u.USUA_NOMB||'(Encargado)' END) as USUA_NOMB , u.USUA_CODI, u.SGD_ROL_CODIGO from usuario u where u.depe_codi=$depeCodi AND u.USUA_ESTA='1' ";
		if($txAccion){
