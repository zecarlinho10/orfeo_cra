...
function _adodb_replace(&$zthis, $table, $fieldArray, $keyCol, $autoQuote, $has_autoinc)
{
		...
		if ($uSet && $where) {
			...
			if ($rs) {
				...
				/*
				 The Select count(1) wipes out any errors that the update would have returned. 
				...
				*/
				...
				# for mysql - which is silly. 
					...
					$cnt = $zthis->GetOne("select count(1) from $table where $where");
	 ...
	 if (!empty($zthis->_nestedSQL) || preg_match("/^\s*SELECT\s+DISTINCT/is", $sql) || 
		...
		if ($zthis->dataProvider == 'oci8') {
			...
			if (preg_match('#/\\*+.*?\\*\\/#', $sql, $hint)) {
				$rewritesql = "SELECT ".$hint[0]." COUNT(1) FROM (".$rewritesql.")"; 
			} else
				$rewritesql = "SELECT COUNT(1) FROM (".$rewritesql.")"; 
			
		} else if (strncmp($zthis->databaseType,'postgres',8) == 0)  {
			$rewritesql = "SELECT COUNT(1) FROM ($rewritesql) _ADODB_ALIAS_";
		}
	} else {		
		// now replace SELECT ... FROM with SELECT COUNT(1) FROM
		$rewritesql = preg_replace(
					'/^\s*SELECT\s.*\s+FROM\s/Uis','SELECT COUNT(1) FROM ',$sql);
		// fix by alexander zhukov, alex#unipack.ru, because count(1) and 'order by' fails 
