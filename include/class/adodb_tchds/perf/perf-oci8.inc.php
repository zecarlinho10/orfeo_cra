...
class perf_oci8 extends ADODB_perf{
	...
	var $tablesSQL = "select segment_name as \"tablename\", sum(bytes)/1024 as \"size_in_k\",tablespace_name as \"tablespace\",count(1) \"extents\" from sys.user_extents 
...
AND    b.name = 'sorts (memory)'",
	...
	'Memory Pools',
		...
		'current connections' => array('SESS',
			'select count(1) from sys.v_$session where username is not null',
			''),
	...
	'Transactions',
		'rollback segments' => array('ROLLBACK',
			"select count(1) from sys.v_\$rollstat",
			''),
	...
	'Parameters',	
		...
		'cursor reuse' => array('CURSOR',
			"select count(1) from (select sql_text_wo_constants, count(1)
  from t1
 group by sql_text_wo_constants
having count(1) > 100)",'These are sql statements that should be using bind variables'),*/
