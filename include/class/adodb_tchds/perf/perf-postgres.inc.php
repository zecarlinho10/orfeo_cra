...
class perf_postgres extends adodb_perf{
	...
	'Ratios',
		'statistics collector' => array('RATIO',
			"select case when count(1)=3 then 'TRUE' else 'FALSE' end from pg_settings where (name='stats_block_level' or name='stats_row_level' or name='stats_start_collector') and setting='on' ",
