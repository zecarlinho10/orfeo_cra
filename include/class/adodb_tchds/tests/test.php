...
*/
		...
		if ($rs && !$rs->EOF) {
			$v = $db->GetOne("SELECT count(1) FROM tab where tname like 'A%'");
  ...
  KEY `myindex` (`row1`,`row2`)
	...
	$db->debug=1;
	$ADODB_FETCH_MODE = ADODB_FETCH_ASSOC;
	$cnt = $db->GetOne("select count(1) from ADOXYZ");
 ...
 //////////////////////////////////////////////////////////////////////////////////////////
	...
	$val = $db->GetOne("select count(1) from ADOXYZ");
	 if ($val == 50) print "<p>GetOne returns ok</p>";
	 else print "<p><b>Fail: GetOne returns $val</b></p>";

	 echo "<b>GetRow Test</b>";
	$ADODB_FETCH_MODE = ADODB_FETCH_NUM;
	$val1 = $db->GetRow("select count(1) from ADOXYZ");
	$val2 = $db->GetRow("select count(1) from ADOXYZ");
 ...
 //	$db->debug=true;
	...
	$db->debug=1;
	$query = $db->Prepare("select count(1) from ADOXYZ");
	...
	if ($i != $db->GetOne('select count(1) from ADOXYZ where id>0 and id<10')) {
