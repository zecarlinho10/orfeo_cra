...
//Limito la busqueda
 ...
 //OBTENEMOS LOS TITULOS y SE QUITAN ESPACIOS 2016/01/28 PARA QUE SE VISUALICEN LA ULTIMA COLUMNA
 $rs_title = $db->conn->SelectLimit($isql,0);
 $a_title =  explode(",", rtrim($rs_title)); //Array en donde guardo los titulos
 $n_title = count($a_title); //Cuento cuantos titulos o columnas tiene
 
echo '<td class="listado2" align="center">Numero  '.$num_total_registros.'</td>';
?>
<!--Ajustado para que imprima en excel se coloca el ID juan carlos villalba 06/01/2016-->
 <table class="table table-bordered table-striped mart-form" align="center" border="2" width="80%" id="Exportar_a_Excel">
      <tbody>
          <tr>
 <?php
...
$rs = $db->conn->SelectLimit($isql,$TAMANO_PAGINA,$inicio);
 ...
 if ($total_paginas > 1) {
    if ($pagina != 1){echo '<a href="'.$url.'?'.$encabezado.'&pagina='.($pagina-1).'"> << Anterior  </a>';}
       ...
       for ($i=1;$i<=$total_paginas;$i++) {
          ...
          echo '  <a href="'.$url.'?'.$encabezado.'&pagina='.$i.'">'.$i.'</a>  ';
    }
if ($pagina != $total_paginas){echo '<a href="'.$url.'?'.$encabezado.'&pagina='.($pagina+1).'"> Siguiente >> </a>';}
