...
if (! defined('_ADODB_LAYER')) {
    ...
    if (defined('ADODB_EXTENSION')) {
        $msg = "Unsupported ADOdb Extension (v" . ADODB_EXTENSION . ") detected! " . "Disable it to use ADOdb";
...
$ADODB_FETCH_MODE, $ADODB_CACHE, $ADODB_CACHE_CLASS, $ADODB_FORCE_TYPE, $ADODB_GETONE_EOF, $ADODB_QUOTE_FIELDNAMES;
 ...
 // / set to true if ADOConnection.Execute() permits binding of array parameters.
		...
		public function setIpClient($ipCliente){
		$this->ipClient = $ipCliente;
		}
...
'))?)(?:\s|$)'; // Whitespace or end of string
                ...
                if (! is_array($sql) && ! $this->_bindInputArray) {
                    ...
                    if (! $array_2d) {
                        ...
                        if (1 != count(array_unique($countElements))) {
                            $this->outp_throw("[bulk execute] Input array has different number of params  [" . print_r($countElements, true) . "].", 'Execute');
                            return false;
                        }
                        unset($countElements);
                    }
                    ...
                    if ($nparams != count($element0)) {
                        $this->outp_throw("Input array has " . count($element0) . " params, does not match query: '" . htmlspecialchars($sql) . "'", 'Execute');
                        return false;
                    }
        ...
        function create_trail($isql)
	{
		if(is_array($isql))
			...
			// descompongo el sql y verifico si es un insert, update o delete
			 $strPos= strpos($sql,"SELECT");
			$arrayQuery = explode(" ", $sql);
            if ( $arrayQuery[0] != "SELECT" || ($strPos === false && $strPos > 10)) {
                ...
                $table = explode(" ", $table);
                $table = $table[0];
                if (strcmp($table, "SGD_AUDITORIA") != 0 && $type != '') {
                    $sql = str_replace("'", "^", $sql);
					$ipLocal = $this->ipClient;
					$usuaDoc = $this->usuaDoc;
                    $query = "INSERT INTO SGD_AUDITORIA (FECHA, USUA_DOC, IP, TIPO, TABLA, ISQL) VALUES ('" . time() . "', '" .  $usuaDoc. "', '$ipLocal', '$type', '$table','" . date("Y-m-d H:i:s") . "> $sql')";
                    $rs = $this->Execute($query);
                    if (! $rs) {
                        $errorPath = ini_get('error_log');
                        error_log("ERROR - ADODB adodb/adodb.inc.php Linea:888 -> Fallo en la funcion create_trail".$query, 0,$errorPath."orfeo.log");
                    }
                }
            }
        }
        ...
        function _Execute($sql, $inputarr = false)
        {
            ...
            if ($this->_queryID === false) {
                error_log( " ".$this->ErrorMsg()." ERROR - ADODB adodb/adodb.inc.php Linea:896 -> Fallo en la funcion EXECUTE ".$sql, 0,"orfeo.log");
                if ($this->debug == 99) {
 ...
 // names of each column in array
        var $_skiprow1;
 // skip 1st row because it holds column names
        var $_fieldobjects;
