...
//Servicio para Buscar un expedietne| un expediente
$server->register('getRadsExpediente',                  //nombre del servicio                 
    array('numeroExpediente' => 'xsd:string',                  //numero de Expediete
     'searchText' => 'xsd:string',                   //parametor a buscar
     ),                               //entradas          
    array('return' => 'xsd:string'),                  // salidas
    $ns                                         //Elemento namespace para el metod       
);
...
/**
 * Permite consultar un expediente por su numero o por alguna etiqueta
 * @param $expNumero, este parametro es el numero de radicado
 * @param $searchText, este parametro es el usuario que crea el expediente, es el usuario de correo
 * @author Jairo Hernan Losada
 * @return Arreglo con los radicados generados a el expediente
 */
function getRadsExpediente($numeroExpediente="", $searchText=""){
  return "ws entro ssss";
  global $ruta_raiz;
  $db = new ConnectionHandler("$ruta_raiz");
  ...
  //Aqui busco la informacion necesaria del usuario para la creacion de expedientes
  //$db->conn->debug = true;
  $sql= "select exp.RADI_NUME_RADI, exp.ra_asun, exp.RADI_FECH_RADI, sexp.sgd_sexp_parexp1
            ,sexp.sgd_sexp_parexp1
            ,sexp.sgd_sexp_parexp2
            ,sexp.sgd_sexp_parexp3
            ,sexp.sgd_sexp_parexp4
           from sgd_sexp_secexpedientes sexp, sgd_exp_expediente exp   
           where  sexp.sgd_exp_numero=exp.sgd_exp_numero AND
