    ...
    if (isset($part->parts)) {
        ...
        if (isset($part->disposition)) {
            if ($part->disposition == "ATTACHMENT" or $part->disposition == "attachment") {
				...
				$partObject =(!empty($part->dparameters))?$part->dparameters:$part->parameters;
                $attachmentDetails = array(
                    "name" => $partObject[0]->value,
                    "partNum" => $partNum,
...
function downloadAttachment($imap, $uid, $partNum, $encoding, $path, $anexName = false, $nurad = false)
{
	...
	$partObject=(!empty($partStruct->dparameters))?$partStruct->dparameters:$partStruct->parameters;
    $filename = mb_decode_mimeheader($partObject[0]->value);
...
{
    if ($section == '1.1') {
        if (is_array($structure->parts[0]->parts[0]->parts[1]->parameters)) {
            $charset = $structure->parts[0]->parts[0]->parts[1]->parameters[0]->value;
        } else {
            $charset = $structure->parts[0]->parts[0]->parts[0]->parameters[0]->value;
        }
    } elseif ($section == '1.2') {
