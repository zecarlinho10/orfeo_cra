...
<form name='forma' id="forma"
      action='marcaEnviar.php?<?="fecha_h=$fechah&dep_sel=$dep_sel&estado_sal=$estado_sal&estado_sal_max=$estado_sal_max&no_planilla=$no_planilla" ?>'
      method=post>
    ...
    while ($i < $num) {
        ...
        if ($estadoFirma == "COMPLETA") {
            ...
            if ($swValido == false) {
                ...
                copy("$ruta_raiz/$linkarchivo", BODEGA."/masiva/$nombreArchivo");
...
if (!$verRegistros) {
    ...
    while (!$rsMarcar->EOF) {
        ...
        //trd y expediente para nueva validacion
        $trd = $rsMarcar->fields["TDOC_CODI"];
        $expediente = $rsMarcar->fields["SGD_EXP_NUMERO"];
        ////////////////////////////////////////
...
<?php   echo("<center><a href='$ruta_raiz/envios/cuerpoMarcaEnviar.php?krd=$krd&usua_perm_impresion=$usua_perm_impresion&carpeta=8&nomcarpeta=Documentos Para Impresion&orderTipo=desc&orderNo=3' target='mainFrame' class='vinculos'>Volver a listado de impresi&oacute;n</a></center>");?>
</form>
</div>
</div>
</div>
