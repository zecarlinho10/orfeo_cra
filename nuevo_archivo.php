...
include_once (realpath(dirname(__FILE__) . "./")."class_control/Direcciones.php");
...
$direcciones = new Direcciones($db);

$isql = "select USUA_LOGIN,USUA_PASW,CODI_NIVEL from usuario where (usua_login = ? ) ";
$rpsql=  $db->conn->prepare($isql);
$rs   = $db->conn->Execute($rpsql,array($krd));
...
<script language="javascript">
 ...
 document.forms['formulario']['tpradic'].value = 'null'
  ...
  function validarGenerico(){
      ...
      if (document.formulario.radicado_salida!=null &&  document.formulario.radicado_salida.checked && document.formulario.tpradic.value=='null'){
          alert ("Debe seleccionar el tipo de radicacion");
          return false;
      }

      archivo=document.getElementById('userfile').value;
      url_carpeta=document.getElementById('url_carpeta').value;
      if (archivo=="" && url_carpeta==""){
          <?php
            if($tipo==0 and !$codigo){echo "alert('Por favor escoja un archivo o digite URL de carpeta en la nube'); return false;";}
...
$dirsDocumentos=$direcciones->getDireccion($numrad);

foreach($dirsDocumentos as $key=>$value){
	if($key == 1)
		$vais ="11";
        else 
		$vais =$key;
	${"nombret_us".$vais}=$value["SGD_DIR_NOMREMDES"];
	${"direccion_us".$vais}=$value["SGD_DIR_DIRECCION"];
	${"dpto_nombre_us".$vais}=$value["DPTO_NOMBRE"];
	${"muni_nombre_us".$vais}=$value["MPIO_NOMBRE"];
}

if (!empty($nombret_us11) and !empty($direccion_us11) and !empty($dpto_nombre_us11) and !empty($muni_nombre_us11)){
    $us_1 = "si"; $usuar=1;
...
if ($is_edit >=1){$sel="selected";}else{$sel="";}
  ...
  //Autos CRA 23/12/2015
if ($tpradic==8 and $valueTp==8)
{
        $comboRadOps =$comboRadOps . "<option value='".$valueTp."' $sel>".$valueDesc."</option>";
$sel="";
}
//CID CRA 23/12/2015
if ($tpradic==4 and $valueTp==4)
{
        $comboRadOps =$comboRadOps . "<option value='".$valueTp."' $sel>".$valueDesc."</option>";
$sel="";
}
...
//Si el radicado es de entrada, siempre va a generar una salida.
if ($tpradic == 1){
if($valueTp == 1 ){
$comboRadOps =$comboRadOps . "<option value='".$valueTp."' $sel>".$valueDesc."</option>";
$sel="";
...
<!-- <p><small class="btn btn-success"><?=$mostrar_mensaje?></small></p> -->
    ...
    <tr align="center">
      <td ><small>URL carpeta nube:</small></td>
      <td align="left" colspan="2" >
        <textarea name="url_carpeta" cols="100" rows="1" class="text" id="url_carpeta"></textarea>
      </td>
    </tr>
    <tr>
