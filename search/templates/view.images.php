...
<li>
	...
	<div class="title imagelist">
		...
		<a target="_blank" rel="noopener noreferrer" href="<?=$id?>"><h2>
		<?php if ($title) { ?>
			<?=$title ?>
		<?php } else { ?>
			<?=$uri_label?>
...
<?=$snippet?>
	...
	<div class="commands">
				...
				<?php } ?>
				<a target="_blank" rel="noopener noreferrer" href="<?=$id?>"><?php echo t('open'); ?></a>
					
				
				<?php if ($uri_tip) { ?>
					</span>
				<?php } ?>
				<?php if ($cfg['metadata']['server']) { ?> | <a target="_blank" rel="noopener noreferrer" title="<?php echo t('meta description'); ?>" href="<?php print get_metadata_uri ($cfg['metadata']['server'], $id); ?>"><?php echo t('meta'); ?></a> <?php } ?> | <?php print '<a target="_blank" rel="noopener noreferrer" href="preview.php?id='.urlencode($id).'">' . t('Preview') . '</a>'; ?>
	</div>
</li>
