...
class Usuario
{
    ...
    /**
     * Variable que contiene el permiso de si un usuario puede tipificar un anexo .
     *
     *
     *
     * tiff
    ...
    /**
     * Variable que contiene el permiso de si un uusairo puede borrar un anexo .
     *
     *
     *
     * tiff
    ...
    {
        $this->cursor = Connection::getCurrentInstance();
    }
    ...
    {
        $q = "select *
        from usuario where usua_login='$login' ";
        ...
        if ($rs && ! $rs->EOF) {
            ...
            $this->usu_cargo = $rs->fields['USUA_CARGO'];
            $this->usua_login = $rs->fields['USUA_LOGIN'];
     ...
     *            el codigo de la dependencia
     * @param $codUsuario string
     *            el codigo del usuario
     * @return boolean
     */
    function usuarioDependecia($dependencia, $codUsuario)
    {
        $q = "select *
        ,usuario.USUA_NOMB
        ,usuario.DEPE_CODI
     ...
     *
     * @param $dependencias es
     *            el grupo de dependencias establecido
     * @return void
     *
     */
     ...
     *            el grupo de dependencias establecido
     * @param $where es
     *            criterio de filtro para los usuarios de las dependencias seleccionadas
     * @return void
     *
     */
     ...
     *
     * @param $values array
     *            arreglo de valores a insertar, cada el elemento de arreglo esta estructurado de la forma Array ( [NOMBRE CAMPO] => VALOR )
     * @return void
     */
    ...
    function usuarioDocto($docto)
    {
        ...
        if ($rs->RecordCount() > 0) {
            ...
            $this->usu_telefono1 = $rs->fields['USU_TELEFONO1'];
            $this->usu_telefono2 = $rs->fields['USU_TELEFONO2'];
            $this->usua_cargo = $rs->fields['USUA_CARGO'];
            $this->usua_codi = $rs->fields['USUA_CODI'];
            $this->perm_tipif_anexo = $rs->fields['PERM_TIPIF_ANEXO'];
        }
    }
        ...
        if ($rs && ! $rs->EOF) {
            ...
            while (! $rs->EOF) {
                if (is_array($usuarios))
                    in_array($rs->fields['USUA_LOGIN'], $usuarios) ? $estDep = 'selected' : $estDep = '';
	...
	<td class='titulos2'>Usuario</td>
            ...
            while (! $rs->EOF) {
                $usuariosNv .= "   <tr>
			<td  class='listado2'>
			<font size=1>" . $rs->fields["USUA_NOMB"] . "</font>
		                              </td>
					<td class='listado2'>&nbsp;
					<font size=1>" . $rs->fields["DEPE_NOMB"] . " " . "</font>
		                             </td>
		                              <td  class='listado2'>&nbsp;
		                              <font size=1>" . $rs->fields["PERFIL"] . "&nbsp;
		                              </font>
    ...
    function findUsuario($buscar, $estado = 1, $limite = 30)
    {
        $busqueda = strtoupper($buscar);
        $usuarios = null;
        $estadoUsuario = "";
        $i = 0;
        if ($estado != - 1) {
            $estadoUsuario = " and usua_esta='" . $estado . "'";
        }
        $query = "select usua_nomb,usua_login,usua_email,d.depe_codi,d.depe_nomb,
        usua_doc,usua_esta,usua_codi,d.id_cont,
        d.id_pais,d.dpto_codi,d.muni_codi
        from usuario u inner join dependencia d on d.depe_codi =u.depe_codi
        where (UPPER(usua_nomb) like '%$busqueda%' or usua_login like'%$busqueda%')
        " . $estadoUsuario;
        if ($limite != 0) {
            $rs = $this->cursor->conn->SelectLimit($query, $limite);
        } else {
            $rs = $this->cursor->query($query);
        }
        
        while (! empty($rs) && ! $rs->EOF) {
            $usuarios[$i] = $rs->fields;
            $i ++;
            $rs->MoveNext();
        }
    ...
    function findJefe($dependencia, $codUsuario = 0)
    {
        $jefe = array();
        if ($codUsuario == self::JEFE) {
            $query = "select usua_nomb,usua_login,usua_email,d.depe_codi,d.depe_nomb,
            usua_doc,usua_esta,usua_codi,
            d.id_cont,d.id_pais,d.dpto_codi,d.muni_codi
            from usuario u inner join dependencia d on d.depe_codi =u.depe_codi
            inner join dependencia  dh on dh.depe_codi_padre=d.depe_codi
            where dh.depe_codi = $dependencia and u.usua_codi=" . self::JEFE;
        } else {
            $query = "select usua_nomb,usua_login,usua_email,d.depe_codi,d.depe_nomb,
            usua_doc,usua_esta,usua_codi,d.id_cont,d.id_pais,d.dpto_codi,d.muni_codi
            from usuario u inner join dependencia d on d.depe_codi =u.depe_codi
            where d.depe_codi = $dependencia and u.usua_codi= " . self::JEFE;
        }
    ...
    function findFirmante($busqueda)
    {
        $firmante = null;
        $query = "select usua_nomb,usua_login,usua_email,d.depe_codi,d.depe_nomb,
					  usua_doc,usua_esta,usua_cargo,usua_codi,
					  d.id_cont,d.id_pais,d.dpto_codi,d.muni_codi
					 from usuario u inner join dependencia d on d.depe_codi =u.depe_codi
						where u.perm_docto_digital = 1";
    ...
    function loadUsuario($usuaLogin)
    {
        $firmante = null;
        $query = "select usua_nomb,usua_login,usua_email,d.depe_codi,d.depe_nomb,
					  usua_doc,usua_esta,usua_codi,
