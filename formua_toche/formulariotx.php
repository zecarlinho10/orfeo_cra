...
//Aqui vamos a poner condicionales para el tipo de $_GET['recibidos'];
	if($pqrsFacebook=="1"){
	...
	if ($_POST['tipoRecibido']=="telefonica"){
		$_SESSION['mrec_codi']=6;
	}
	if ($_POST['tipoRecibido']=="personal"){
		$_SESSION['mrec_codi']=7;
	}
...
//	}
	...
	$_SESSION['usuario']=1;
	if(!$_SESSION['dependencia']) $_SESSION['dependencia']=321;
	$dependenciaRad = $_SESSION['dependencia'];
	$_SESSION['radicado']=$_GET['radicado'];
	$_SESSION['documento_destino']=$_GET['documento_destino'];$secRadicaFormularioWeb="secr_tp2_321";//.$secRadicaFormularioWeb;

	$numseco=$db->conn->GenID($secRadicaFormularioWeb);
	$numero_completado = "000000".$numseco;
	...
	$depeRadicaFormularioWeb;  // Es radicado en la Dependencia zzz
	$depeRecibeFormularioWeb;  // Es recibido en la Dependencia yyy
	...
	if($tipoDocumento > 0 && $tipoDocumento != 5 ){
		...
		$tipdoc= $tipoDocumento-1;//$db->conn->debug=true;
		$ins_ciu="insert into sgd_ciu_ciudadano values($tipdoc,".$num_ciu.",'".mb_strtoupper($_SESSION['nombre_remitente'],"utf-8")."','".mb_strtoupper($_SESSION['direccion_remitente'],"utf-8")."','".mb_strtoupper($_SESSION['apellidos_remitente'],"utf-8")."','','".$_SESSION['telefono_remitente']."','".$_SESSION['email']."',".$_SESSION['muni'].",".$_SESSION['depto'].",'".$_SESSION['cedula']."','170','1')";
	...
	values(".$num_dir.",1,".$num_oem.",0,$numeroRadicado,0,".$_SESSION['muni'].",".$_SESSION['depto'].",'".$_SESSION['direccion_remitente']."','".$_SESSION['telefono_remitente']."',0,'".mb_strtoupper($_SESSION['nombre_remitente'],"utf-8")." ".mb_strtoupper($_SESSION['apellidos_remitente'],"utf-8")."','".mb_strtoupper($_SESSION['nombre_remitente'],"utf-8")." ".mb_strtoupper($_SESSION['apellidos_remitente'],"utf-8")."',1,'".$_SESSION['nit']."','".$_SESSION['email']."')";
		...
		//Anonimo
	$_SESSION['muni']=($_SESSION['muni']==0)?1:$_SESSION['muni'];
	$_SESSION['depto']=($_SESSION['depto']==0)?1:$_SESSION['depto'];
		$num_ciu=$db->conn->GenID('SEC_CIU_CIUDADANO');
	...
	$_POST['entidad']=empty($_POST['entidad'])?0:$_POST['entidad'];
...
values ($numeroRadicado, sysdate,'$tipoSolicitud',".$_SESSION['mrec_codi'].",".$_POST['entidad'].",
...
,'". $descripcionAnexos ."', ";
	...
	//$usuaRadicaFormularioWeb =  $_SESSION['usuaRadicaFormularioWeb'];
	$anoRad = date("Y");
	...
	,'$depeRadicaFormularioWeb','$usuaRadicaFormularioWeb',5,1,0,0,1,0,0,0,0,0,'".$_SESSION['codigoverificacion']."',".$_SESSION['depeRadicaFormularioWeb'].",2)";
	...
	//Inserta historico
	$obsHistorico=!empty($_POST['usua_login'])?'Usuario radicador: '.$_POST['usua_login']:'Formulario Web PQRS';
...
values($dependenciaRad,sysdate,$usuaRadicaFormularioWeb,$numeroRadicado,'$obsHistorico',".$_SESSION['usuario'].",'22222222',2,'".$_SESSION['documento_destino']."',".$_SESSION['dependencia'].")";
	...
	$sql_login="select usua_login from usuario where usua_codi=".$_SESSION["usuaRecibeWeb"]." and depe_codi=".$_SESSION["depeRecibeFormularioWeb"];
...
values(".$numeroRadicado.",".$numeroRadicado.sprintf("%05d",($i+1)).",".$tipoCodigo.",".$uploader->sizes[$i].",'S','".$rs_login->fields['USUA_LOGIN']."','".$uploader->sha1sums[$i]."',1,'".$uploader->nombreOrfeo[$i]."','N',0,0,0,1,1,".$_SESSION["depeRadicaFormularioWeb"].",sysdate,0)";
...
$pdf->Text(140,44,textoPDF("Rem/D: ". substr($_SESSION['nombre_remitente'],0,10)." ".substr($_SESSION['apellidos_remitente'],0,10)));
$pdf->SetFont('Arial','',7);
//$pdf->Text(110,40,textoPDF(textoPDF($entidad_largo)));
$pdf->Text(123,41,textoPDF(date('d')."-".date('m')."-".date('Y')." ".date('h:i:s')) . "   Folios: N/A (WEB)   Anexos: ". $_SESSION['cantidad_adjuntos'] );
$pdf->SetFont('Arial','',7);
//$pdf->Text(110,48,textoPDF("Consulte el estado de su trámite en nuestra página web http://www.correlibre.org"));
$pdf->Text(120,47,textoPDF("Visite nuestra página web $httpWebOficial"));
...
$pdf->SetFont('','B');
$pdf->Text(12,85,textoPDF(html_entity_decode($entidad_largo,ENT_QUOTES,"UTF-8")));
$pdf->SetFont('','');
...
$db->conn->Execute($sqlu);
    ...
    $usMailSelect  = "orfeo@cra.gov.co"; 
    $mail->IsSMTP(); // telling the class to use SMTP
    //$mail->AddReplyTo($usMailSelect);
    $mail->SetFrom($usMailSelect);
    ...
    $mail->Subject    = "Radicado " .$numeroRadicado . "CRA";
...
<title>OrfeoPQRDS <?=$entidad_largo?></title>
...
<table width="80%" border="0" align="center" cellpadding="0"
	...
	<tr>
		<td align="center"><br />
		<img src="../logoEntidad.png" width="100%" /></td>
	</tr>
	<tr>
		<td>&nbsp;</td>
	</tr>
