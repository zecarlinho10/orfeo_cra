function valida_form() {
		...
		function checkear(idactuacionactividad,idActividad){
		    var valor=1
		    var cierra=1
		    var idactuacionactividad=idactuacionactividad;
		    if( $("#txtFinalizado"+idactuacionactividad).prop('checked') ) {
                valor = 1;
                
                if(idActividad==129){
                	if(confirm("Confirma el cierre definitivo de la actuacion.?"))
				        cierra=1;
				    else cierra=0;
				}                	
            }
		    ...
		    var parametros = {
                "idactuacionactividad" : idactuacionactividad,
                "valor" : valor,
                "cierra" : cierra
	        };
				$.ajax({
		...
		function eliminaActividad(actividadid){
	        ...
	        $.ajax({
	                data:  parametros, //datos que se envian a traves de ajax
	                url:   '../actuacion-actividad/eliminaActividad.php', //archivo que recibe la peticion
	                type:  'post', //método de envio
	                beforeSend: function () {
	                        $("#resultado").html("Procesando, espere por favor...");
	                },
...
.ready(
		...
		$("#txtActuacion").change(function(){
			$.get("getFechaActuacion.php","txtActuacion="+$("#txtActuacion").val(), function(data){
				$("#fechaFinActuacion").html(data);
				console.log(data);
			});
			$.get("getActividades.php","txtActuacion="+$("#txtActuacion").val(), function(data){
				$("#tabla").html(data);
				console.log(data);
			});
		});
