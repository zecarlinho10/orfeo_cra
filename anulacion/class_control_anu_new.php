	 ...
	 function tabla_sql($query)
	 {
		...
		if ($jh_db) 
		{
		   $numreginf = 0;
			...
			//echo "<p><span class=listado2>Fechas Inicio " . $fecha_ini." hasta " . $fecha_fin." </span>";
		}
		global $ADODB_FETCH_MODE;
		$ADODB_FETCH_MODE = ADODB_FETCH_ASSOC;
		$jh_db = $this->cursor->conn->Execute($query);
	    $tabla_html = "<table border=1 width=100% >";
	    echo "<table class='borde_tab' width=100% id='Exportar_a_Excel'>";
		if ($jh_db)
		  {
			...
			foreach($this->campos_vista as $campos_d)
			  {
				...
				$tabla_html .=  "<td width='$width' height=80 bgcolor='#CCCCCC'><center>$campos_d</td>";
				...
				foreach($this->campos_tabla as $campos_d)
				  {
					...
					$tabla_html .=  "<td  align=$align width=".$this->campos_width[$iii]." height=80>$dato</td>";
