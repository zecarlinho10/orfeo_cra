...
<?php
      include_once (realpath(dirname(__FILE__) . "/../")."/js/funtionImage.php");       
?>
</head>
...
$whereFiltro=$dependencia_busq2;
	...
	?>
  <form name=formEnviar action='../anulacion/solAnulacion.php?<?="krd=$krd" ?>&tpAnulacion=<?=$tpAnulacion?>&depeBuscada=<?=$depeBuscada?>&estado_sal_max=<?=$estado_sal_max?>&pagina_sig=<?=$pagina_sig?>&dep_sel=<?=$dep_sel?>&nomcarpeta=<?=$nomcarpeta?>&orderTipo=<?=$orderTipo?>&orderNo=<?=$orderNo?>' method=post>
 <?
