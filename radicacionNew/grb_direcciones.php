<?php
    ...
    if (!is_null($muni_us1)) {
        ...
        switch (count($muni_tmp1)) {
            case 4:
                ...
                break;
            case 3:
                $idcont1 = (!empty($tmp_idcont)) ? $tmp_idcont : $idcont1;
                $idpais1 = $muni_tmp1[0];
    ...
    if(!$modificar) {
       ...
       $numRandom = rand(10,99);
       $nextval = $nextval . $numRandom;
    }
    if ($nextval==-1) {
        echo "<span class='etextomenu'>No se encontro la secuencia sec_dir_drecciones ";
    }
        //Es otro destinatario y no trae documento
        
        
    if( ($documento_us1 == null or $documento_us1 == 0) && $documento_us7 != null) {
    $documento_us1 = $documento_us7;
    $direccion_us1 = $direccion_us7;
    $telefono_us1 = $telefono_us7;
    $sgd_ciu_codigo = $sgd_ciu_codi;
    $sgd_oem_codigo = $sgd_oem_codi;
    $sgd_esp_codigo = $sgd_esp_codi;
    }

    global $ADODB_COUNTRECS;
    // Si no se coloca un destinatario diferente el tomaria el nombre de la persona para grabarla en sgd_dir_drecciones.
    if(!$otro_us7 and $nombre_us1) $otro_us7= $nombre_us1;
    if($documento_us1  and substr($cc,0,1)!="G") {
        $sgd_ciu_codigo = 0;
        $sgd_oem_codigo = 0;
        $sgd_esp_codigo = 0;
        $sgd_fun_codigo = 0;
        ...
        switch ($insertSQL) {
            case 1:	{	//Insercion Exitosa
			            ...
			            //echo '<font size="1">Ha sido agregado el destinatario con exito.</font>';
                        $newId=true;
                    }break;
            case 2:{	//Update Exitoso
			            //echo '<font size="1">Ha sido actualizado el destinatario con exito.</font>';
                        $newId = false;
                    }break;
            case 0:{	//Error Transaccion.
                        ...
                        echo "<font color=red aling=center><b>No se ha podido actualizar la informacion de SGD_DIR_DRECCIONES</b> </font><!-- $isql -->";
                    }break;
        }
        unset($record);
    ...
    if($documento_us2) {
        ...
        $rsg = $conexion->conn->Execute($isql);
        ...
        if 	($rsg->EOF) {
            ...
            $isql = "insert into SGD_DIR_DRECCIONES(SGD_TRD_CODIGO,
                                            ...
                                            values('$sgdTrd',
                                                    ...
                                                    $nurad,
                                                    0,
                                                    '".trim($direccion_us2) . "', '".trim($telefono_us2)."', '$mail_us2',
                                                    2,
                                                    $nextval,
                                                    '$otro_us2')";
