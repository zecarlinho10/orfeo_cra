<?php
	...
	if($espcodi) {$isql = "select NIT_DE_LA_EMPRESA as SGD_CIU_CEDULA,
...
//		$rs = $db->query($isql);
	    if(!$rs->EOF) {
	    		echo "<hr><br />Hay datos en DB 1<br />";
...
//     die("Debio ejecutar: $isqlRemDest");
     if (!$rsRemDest->EOF){                                                                                 
       $rem_destinoBusq = $rsRemDest->fields['SGD_REM_DESTINO'];                                        
 	}
    if (!isset($numrad)) {
    	$checkVal = $_POST["checkValue"];
    	$arrayValues = array_values($checkVal);
    	
//    	die("check: " . $arrayValues[0]);
		$numrad = $arrayValues[0];
    }
	if($rem_destinoBusq==7) {
			echo "<hr><br />Es tipo 7<br />";
			echo ("Anexo: " . $anexo);

//	  $rem_isql = " and sgd_dir_tipo='701' ";	
	  $rem_isql = " and sgd_anex_codigo=$anexo ";	
         ...
         if (!$rsRemDest7->EOF){             
            $rem_destino7 = $rsRemDest7->fields['SGD_DIR_TIPO'];             
        	$rem_isql = " and sgd_dir_tipo='$rem_destino7'";  
         }else{                                             
        	$rem_isql = "";  
          }
    }
    ...
    while(!$rs->EOF&&$rs!=false) {
	   ...
	   if($fun!=0) { 
            ...
            $isql = "select a.USUA_NOMB as SGD_CIU_NOMBRE,
                         ...
                         FROM USUARIO a,
                                dependencia b
                         where  a.depe_codi = b.depe_codi and 
                                a.USUA_DOC = '$fun' AND";
			$rs1 = $db->conn->Execute($isql);	            
 			$tipo_emp = 6;
	   ...
	   if($rem_isql) {
//	   		die("Pasa por rem_isql: " . $rem_isql);
