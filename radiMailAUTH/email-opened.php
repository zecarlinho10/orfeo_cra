...
if($uid){
	...
	if (empty($nurad) or !isset($nurad)){
		...
		include "mensaje.php";
		//include "mensaje_final.php";
		$data = ob_get_flush();
		$data = str_replace($links,"",$data);
		//$data = str_replace("../","",$data);
		$fp = fopen('../bodega/tmp/tmp_'.$nurad.'.html', 'w');
		fwrite($fp, $data);
		fclose($fp);
		$ano=substr($nurad,0,4);
		//echo "$data";
		//$pathBodega="/$ano/$dependencia/$nurad.html";
		$pathBodega="/$ano/$dependencia/$nurad.pdf";
		$pathFileDOCX="../bodega/tmp/tmp_$nurad.html";
		$pathDocumentConvertedPDF="../bodega/tmp/";
		//$execommand = "libreoffice --headless --convert-to pdf $pathFileDOCX --outdir  $pathDocumentConvertedPDF -env:UserInstallation=file:///tmp/LibreOffice_Conversion_${USER} ";
		$execommand = "htmldoc --webpage -f ../bodega/tmp/tmp_$nurad.pdf $pathFileDOCX";
		$retorno = shell_exec($execommand);
		$command = "mv ../bodega/tmp/tmp_$nurad.pdf ../bodega$pathBodega";
		$retorno = shell_exec($command);
		//if (copy("../bodega/tmp/$date.html","../bodega/$pathBodega")){
		if (is_file("../bodega/$pathBodega")){
