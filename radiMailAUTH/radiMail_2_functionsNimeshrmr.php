...
function getAttachments($imap, $mailNum, $part, $partNum) {
	...
	if (isset($part->parts) && $part->subtype != "RFC822") {
		...
		if ($part->disposition == "ATTACHMENT" || $part->disposition == "attachment" || ($part->disposition == "INLINE" && $part->subtype == "PDF")) {
			$partStruct = imap_bodystruct($imap, $mailNum,
					$partNum);
			if ($part->dparameters[0]->value && $part->dparameters[0]->attribute=='FILENAME'){
				$attName=$part->dparameters[0]->value;
			}elseif ($part->dparameters[1]->value && $part->dparameters[1]->attribute=='FILENAME'){
				$attName=$part->dparameters[1]->value;
...
function downloadAttachment($imap, $uid, $partNum, $encoding, $path, $anexName=false, $nurad=false, $auto_name=false,$ruta_raiz="/var/www/html/orfeo7/") {
	...
	if (!$anexName and !$nurad and !$auto_name){
		...
		if ($auto_name){
			$url = "$ruta_raiz/bodega/tmp/radimail/$auto_name";

			$fopen = fopen($url, 'w');
			fwrite($fopen, $message);
			fclose($fopen);
		}else{
			$url = "$ruta_raiz/bodega/".substr($nurad,0,4) ."/".$_SESSION["dependencia"] ."/docs/$anexName";
			$fopen = fopen($url, 'w');
			fwrite($fopen, $message);
			fclose($fopen);
		}
