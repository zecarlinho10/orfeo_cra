...
/*  Fabian Mauricio Losada Florez      12-Octubre-2006 						     */
/*************************************************************************************/
$krdOld = $krd;
$per=1;
session_start();
...
$t=0;
$db->conn->debug=true;
$rm=$db->conn->Execute("select distinct(radi_nume_radi),info_codi from informados");
while(!$rm->EOF){
$inf=$rm->fields['INFO_CODI'];
$rad=$rm->fields['RADI_NUME_RADI'];
if($inf==""){
echo $t++;
$db->conn->debug=true;
$db->conn->Execute("update informados set info_codi=33675670 where radi_nume_radi='$rad'");
}
$rm->MoveNext();
}*/
/*
