...
if($_GET['action']=='muni'){
    ?><select  class='select' id="mun" name="codMuni2" onchange="edificio()"><option>-- Seleccione --</option><?
	 echo       municipio($db,$_GET['depto']);
		  	?></select><?
}

if($_GET['action']=='edificio'){
    ?><select class='select' id="edif" name="exp_edificio2" onchange="piso('piso')"><option>-- Seleccione --</option><?
          echo       edificio($db,$_GET['depto'],$_GET['muni']);
		  	?></select><?
}
if($_GET['action']=='piso'){
...
//		   echo $_GET['padre'];
	   ...
	   $nombre=wait($db,$_GET['padre']);
	   		   if($nombre[1]==Null){
		   echo "Numero de Ubicaci&oacute;n virtual ".$_GET['padre'];
?><input type="hidden" name="exp_entre" value="<? echo $_GET['padre']; ?>">
<input type="hidden" name="exp_caja" value="<? echo $_GET['padre']; ?>"><?
		   }
		   else{		   
 ...
 * @return String
 */
function edificio($db,$codiDepto,$codiMuni){
