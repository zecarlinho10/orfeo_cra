...
/*  Fabian Mauricio Losada Florez      20-Noviembre-2006 						     */
/*************************************************************************************/
 $krdOld = $krd;
session_start();
...
?>
<td width="20%" class='titulos2'> <p>AG 
  <input name="buscar_rete" type="checkbox" class="select" value="1" <?=$datoss?>>
  &nbsp;&nbsp;
  <?
	if($buscar_rete == 2) $datoss = "checked"; else $datoss= "";
?>
  AC 
  <input name="buscar_rete" type="checkbox" class="select" value="2" <?=$datoss?>>
...
<td width="20%" class='titulos2' align="left">CONSECUTIVO </td>
	<td width="20%" class='titulos2'><input type=text name=buscar_consecu value='<?=$buscar_consecu?>' class="tex_area">
    </td>
  </tr>
...
OR s.SGD_SEXP_PAREXP5 LIKE '%$buscar_parametros%' OR s.SGD_SEXP_PAREXP6 LIKE '%$buscar_parametros%'";$l="and";}
else {$param="";$l="";}
if($buscar_consecutivo!=""){$conse="e.SGD_EXP_CARPETA LIKE '$buscar_consecutivo'";$n="and";}
else {$conse="";$n="";}
if($buscar_ufisica!=""){
if($item2=="AREA ")$archi="e.SGD_EXP_UFISICA LIKE '$buscar_ufisica'";
...
if($buscar_consecu!=""){$nref="e.SGD_EXP_NREF LIKE '%$buscar_consecu%'";$w="and";}
else {$nref="";$w="";}
if ($fecha!="" or $fechafin!="")$orde=" order by e. sgd_exp_fech_arch";
else $orde=" order by e. sgd_exp_numero";

$at=$buscar_exp.$buscar_rad.$buscar_piso.$buscar_caja.$buscar_estante.$buscar_caja.$buscar_caja2.$buscar_rete.$fecha.$fechafin.$buscar_parametros.$buscar_consecu.
$buscar_consecutivo.$buscar_ufisica.$codserie.$codiSBRD.$codProc;
$bt=$buscar_exp.$buscar_rad.$buscar_piso.$buscar_caja.$buscar_estante.$buscar_caja.$buscar_caja2.$buscar_rete.$fecha.$fechafin.$buscar_parametros.$buscar_consecu.
...
if(($buscar_docu!="" and $at=='000')){
	...
	$parametro4=$rsr->fields['SGD_SEXP_PAREXP4'];
	$parametro5=$rsr->fields['SGD_SEXP_PAREXP5'];
	$parametro6=$rsr->fields['SGD_SEXP_PAREXP6'];
...
<td class=leidos2 align="center"><b><span class=leidos2><?=$nref1?></b></td>
...
$rsr->MoveNext();
	...
	while(!$rs->EOF){
		...
		$parametro4=$rs->fields['SGD_SEXP_PAREXP4'];
		$parametro5=$rs->fields['SGD_SEXP_PAREXP5'];
		$parametro6=$rs->fields['SGD_SEXP_PAREXP6'];
