<?
    ...
    include_once "$ruta_raiz/jh_class/funciones_sgd.php";
           ...
           $nomRemDes["x1"]="";
           $nomRemDes["x2"]="";
    while(!empty($rs) &&   !$rs->EOF && $rs!=false) 
	...
	if(!$ik)
	   ...
	   $otro = trim($rs->fields["SGD_DIR_NOMBRE"]);
           $nomOtro["x".$ik] = trim($rs->fields["SGD_DIR_NOMBRE"]);
	   $nomRemDes["x".$ik] = trim($rs->fields["SGD_DIR_NOMREMDES"]);
	   $dirDireccion["x".$ik] = trim($rs->fields["SGD_DIR_DIRECCION"]);
	   $dirTel["x".$ik] = trim($rs->fields["SGD_DIR_TELEFONO"]); 
	   $dirEmail["x".$ik] = trim($rs->fields["SGD_DIR_MAIL"]); 	   
	   $dirCont["x".$ik] = $rs->fields["ID_CONT"];
	   $dirPais["x".$ik] = $rs->fields["ID_PAIS"];
           $dirDptoCodi["x".$ik] = $dpto;
	   $dirMuniCodi["x".$ik] = $muni;
	   $codigo_pai_dep =  $rs->fields["ID_PAIS"]."-".$rs->fields["DPTO_CODI"];
	   $a = new LOCALIZACION($codigo_pai_dep,$rs->fields["MUNI_CODI"],$db);
	#$a = new LOCALIZACION($rs->fields["DPTO_CODI"],$rs->fields["MUNI_CODI"],$db);
	   $dpto_nombre_us1 = $a->departamento;
           $muni_nombre_us1 = $a->municipio;
	   $dirDpto["x".$ik] = $a->departamento;
	   $dirMuni["x".$ik] = $a->municipio;
	...
	//	 print("IK VALE($ik)");
	   ...
	   if($fun!=0)
	   { 
	    $codiATexto = "a.USUA_EXT";// $db->conn->numToString("a.USUA_EXT");
	   ...
	   if($ik==1)
		  {
			...
			$nombre_us1=$nombre;
                        //echo "valor del nombre es : ".$nombre;
...
if(isset($direccion_us1)){$direccion_us1=$dirDireccion["x1"];}
if(isset($codep_us1)){$codep_us1=trim($dirDptoCodi["x1"]);}
if(isset($muni_us1)){$muni_us1 = trim($dirMuniCodi["x1"]);}

if(!trim($nombre_us1) && $ik==1 && $nomOtro["x1"])      	$nombre_us1 = trim($nomOtro["x1"]);
if(!trim($nombre_us2) && $ik==2 && $nomOtro["x2"])      	$nombre_us2 = trim($nomOtro["x2"]);

if(!trim($direccion_us1) && $ik==1)  	$direccion_us1 = trim($dirDireccion["x1"]);
if(!trim($direccion_us2) && $ik==2)  	$direccion_us2 = trim($dirDireccion["x2"]);
if(!trim($mail_us1) && $ik==1)  	$mail_us1 = trim($dirEmail["x1"]);
if(!trim($mail_us2) && $ik==2)  	$mail_us2 = trim($dirEmail["x2"]);
if(!trim($telefono_us1) && $ik==1)  	$telefono_us1 = trim($dirTel["x1"]);
if(!trim($telefono_us2) && $ik==2)  	$telefono_us2 = trim($dirTel["x2"]);
if(!trim($muni_us1) && $ik==1)  	$muni_us1 = trim($dirMuniCodi["x1"]);
if(!trim($muni_us2) && $ik==2)  	$muni_us2 = trim($dirMuniCodi["x2"]);
if(!trim($codep_us1) && $ik==1)  	$codep_us1 = trim($dirDptoCodi["x1"]);
if(!trim($codep_us2) && $ik==2)  	$codep_us2 = trim($dirDptoCodi["x2"]);
if(!trim($idpais1) && $ik==1)  		$idpais1 = trim($dirPais["x1"]);
if(!trim($idpais2) && $ik==2)  		$idpais2 = trim($dirPais["x2"]);
if(!trim($idcont1) && $ik==1)  		$idcont1 = trim($dirCont["x1"]);
if(!trim($idcont2) && $ik==2)  		$idcont2 = trim($dirCont["x2"]);	 
if(!trim($cc_documento_us1) && $ik==1)  $cc_documento_us1 = trim($dirDoc["x1"]);
if(!trim($cc_documento_us2) && $ik==2)  $cc_documento_us2 = trim($dirDoc["x2"]);
