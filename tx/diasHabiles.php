...
function diasHabiles($fechaIni,$fechaFin)
{
	...
	{   
		...
		$fechaTMP = mktime(0, 0, 0, intval($mes)  , intval($dia)+intval($kDia), intval($ano));
		$diaDelMes = date("w",mktime(0, 0, 0, intval($mes)  , intval($dia)+intval($kDia), intval($ano)));
	...
	$x=(isset($a))?-$cDiasSumar+1:$cDiasSumar;
	if($x<=0) return $x;
	else return $x-1 ;
}
