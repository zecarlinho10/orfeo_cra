...
<body bgcolor="#FFFFFF">
	<form method="POST" action="expediente.php" name="TipoDocu"
		class="smart-form">
		<input type=hidden name=numrad value=<?=$numrad?>> <input type=hidden
			name=nurad value=<?=$nurad?>> <input type=hidden name=num_expediente
		...
		<table width="80%" border="0" cellspacing="1" cellpadding="0"
			...
			<tr>
				...
				<td width="38%" class=listado5><select name=nivelExp class=select>
<?
if ($nivelExp == 0)
	$datoss = " selected ";
else
...
if ($nivelExp == 1)
	...
	$datoss = "";
?>
<option value=1 <?=$datoss?>>Privado solo jefe y Ususarios Actuales de
							Radicados</option>
<?
if ($nivelExp == 2)
	$datoss = " selected ";
	...
	$datoss = "";
?>
<option value=2 <?=$datoss?>>Privado dependencia (Solo usuarios
							Dependencias)</option>
<?
if ($nivelExp == 3)
	$datoss = " selected ";
	...
	$datoss = "";
?>
<option value=3 <?=$datoss?>>Privado Solo Usuario Responsable Y Jefe (No
							Usuarios actuales)</option>
				</select></td>
			</tr>
			<tr>
				<TD class=listado5 COLSPAN=2><center>Si selecciona Privado, La
						Dependencia de la persona que posee el Expediente ser&aacute; la
						&uacute;nica que podr&aacute; ver los radicados de dicho
						Expediente.</TD>
			</tr>
			</center>
			<tr>
				<td class=listado5 align="center">
					<center>
						<input type="submit" class="botones" name=grbNivel
							value="Grabar Nivel">
				
				</td>
				<td class=listado5 align="center"><input name="Cerrar" type="button"
					class="botones" id="envia22"
					onClick="opener.regresar();window.close();" value="Cerrar">
				</center></td>
			</tr>
			<tr>
