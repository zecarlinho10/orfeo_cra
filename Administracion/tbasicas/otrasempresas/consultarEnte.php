<?php
/*Modulo de Consulta de Entidad
  Creado por: Ing.Juan Carlos Villalba
*/
//session_start();
...
if($usModo ==3) $tituloCrear = "Consultar Ente";
//$db->conn->debug=true;
$error = 0;
?>
<html>
...
function envio_datos()
{
	if(isWhitespace(document.forms[0].txt_esp.value))
		{	alert("El campo Nombre Ente no ha sido diligenciado.");
...
<?
	...
	//$db->conn->debug = true;
?>
<form action="consultarEnte.php?<?=$params?>" method="post" enctype="multipart/form-data" name="formEsp" id="formEsp" onSubmit="return envio_datos();">
...
<tr bordercolor = "#FFFFFF">
<td width="20%" align="center" class="titulos2">NOMBRE DEL ENTE</td>
...
{
$sqlesp="select e.SGD_OEM_CODIGO,
         e.SGD_OEM_OEMPRESA,
         e.SGD_OEM_REP_LEGAL,
         e.SGD_OEM_NIT,
         e.SGD_OEM_DIRECCION,
         ...
         m.muni_nomb,
         p.nombre_pais
         from SGD_OEM_OEMPRESAS e, sgd_def_paises p, departamento d, municipio m
         where  e.DPTO_CODI=d.DPTO_CODI
	     and e.MUNI_CODI=m.MUNI_CODI
	     and d.DPTO_CODI=m.DPTO_CODI
	     and e.ID_PAIS = p.ID_PAIS
	     and e.SGD_OEM_OEMPRESA like '%".strtoupper($_POST['txt_esp'])."%'";
	//$db->conn->debug = true;
	$rs=$db->query($sqlesp);
	if($rs && !$rs->EOF)
	{
...
?>
	<tr class="grisCCCCCC" align="center"> 
	<td width="11%" CLASS="titulos5" ><font size="-3"><?=$rs->fields['SGD_OEM_CODIGO']?></font></td>
	<td width="11%" CLASS="titulos5" ><font size="-3"><?=$rs->fields['SGD_OEM_OEMPRESA']?></font></td>
	<td width="14%" CLASS="titulos5" ><font size="-3"><?=$rs->fields['SGD_OEM_REP_LEGAL'] ?></font></td>
	<td width="15%" CLASS="titulos5" ><font size="-3"><?=$rs->fields['SGD_OEM_NIT']?></font></td>
	<td width="14%" CLASS="titulos5"><font size="-3"><?=$rs->fields['SGD_OEM_DIRECCION']?></font></td>
	<td width="9%" CLASS="titulos5" ><font size="-3"><?=$rs->fields['SGD_OEM_TELEFONO']?></font></td>
