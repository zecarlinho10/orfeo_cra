...
class TipoDocumental
{
    ...
    function setFechVenci($noRadicado, $tdoc)
    {
        ...
        $fechaVencimiento = date("Y-m-d",$this->calendario->calcular($fechRad, $tiemTdoc));
         ...
         */
        $record = array();
        $record['FECH_VCMTO'] = $fechaVencimiento;//$resul;
        $updateSQL = $this->db->conn->GetUpdateSQL($rsTmp, $record, true);
        // Actualiza el registro en la base de datos
        $this->db->conn->Execute($updateSQL);
        //$this->db->conn->Replace("radicado",$record,array("RADI_NUME_RADI"),false);
     ...
     *
     * @param
     *            int dependencia Dependencia que olicita la transaccion
     * @param
     *            int usuadoc Documento de identificaci�n del usuario que solicita la transaccion
     * @return void
     * @access public
     */
    function insertarTRD($codiTRDS, $codiTRD, $noRadicado, $coddepe, $codusuario, $tdoc = null)
    {
        ...
        if (! is_array($codusuario)) {
            ...
            $usDoc = $codusuario["USUA_DOC"];
            $codusua = $codusuario["USUA_CODI"];
        }
