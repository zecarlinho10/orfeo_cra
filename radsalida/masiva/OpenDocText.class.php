 ...
 */
 ...
 /* Variables privadas */
	...
	function setWorkDir ($workDir) {
		...
		if (is_dir($workDir)) {
			...
			echo $error;
				...
				$this->workDir = str_replace("\\", "\\\\", getcwd()) . DIRECTORY_SEPARATOR;
    ...
    function abrirOdt()
    {
		$nombreDir = array();
		$verificacion = '';
		$dirTmp = str_replace("\\", "\\\\", getcwd()) . DIRECTORY_SEPARATOR;
		$creoDir = false;
		$existeDir = false;
			...
			{	$nombreDir = explode('.',$this->nombreOdt);
				...
				if (!$existeDir)
				{
					...
					{	( $this->debug == true) ? ( $error = "<CENTER><table class=borde_tab><tr><td class=titulos2>Cre&oacute el directorio: " . $this->cacheDir . $nombreDir[0]. " todo ok</td></tr></table>" ) : $error = '' ;
						...
						{	case 'W':
							{
								$varTemp = "cmd /C ".dirname(__FILE__). DIRECTORY_SEPARATOR ."unzip.exe -o " . $this->cacheDir . $nombreDir[0] . ".zip" ." -d " . $this->cacheDir . $nombreDir[0] . DIRECTORY_SEPARATOR;
								$WshShell = new COM("wscript.shell");
    ...
    function salvarCambios( $archivoTmp, $archivoFinal, $tipoUnitario )
	{
		...
		if ($esArchivo)
		{
			...
			if ($this->fp)
			{
			    ...
			    switch ($this->SO)
				{	case 'W':
					{	$varTemp = "cmd /C chdir /D ". $this->workDir.$nombreZip[0]." && ".dirname(__FILE__). DIRECTORY_SEPARATOR."zip.exe -rq " . $this->workDir . $nombreZip[0] . ".odt *";
						$WshShell = new COM("WScript.Shell");
						$verificacion = $WshShell->Run($varTemp, 0, true);
						$WshShell = null;
					}break;
				...
				$intBodega = strpos($this->workDir, DIRECTORY_SEPARATOR."bodega"); //HLP OJO HLP
				if ( $intBodega === false )
				{
				...
				if ($vieneTemp != null)
				{
					...
					switch ($this->SO)
					{	case 'W':
						{	$cmd_cp = "cmd /C copy /Y " . $this->workDir . $nombreZip[0] . ".odt " . $workDIR.DIRECTORY_SEPARATOR."masiva".DIRECTORY_SEPARATOR."tmp_".$nombreZip[0]. ".odt ";
							$WshShell = new COM("WScript.Shell");
				...
				else
				{
					...
					{	
						...
						{	case 'W':
							{
								$cmd_cp = str_replace('/', DIRECTORY_SEPARATOR, $cmd_cp);
					...
					{	$cmd_cp = "cp " . $this->workDir . $nombreZip[0] . ".odt " . $workDIR .date('Y')."/".str_pad($_SESSION['dependencia'],3,"0", STR_PAD_LEFT)."/docs/".$nombreZip[0]. ".odt "   ;
						switch ($this->SO)
						{	case 'W':
							{	$cmd_cp = "cmd /C copy /Y " . $this->workDir . $nombreZip[0] . ".odt " . $workDIR .  str_replace("/", DIRECTORY_SEPARATOR , str_replace("//","", $archivoFinal));
								$WshShell = new COM("WScript.Shell");
								$verificacion = $WshShell->Run($cmd_cp, 0, true);
								$WshShell = null;
							}break;
