 ...
 /* Variables privadas */
  ...
  var $debug = false;
    ...
    exec( "rm -rf " . $workDir . "/" . $this->directorio . "*" );
		...
		if (is_dir($workDir)) {
			...
			echo $error;
				...
				$this->cacheDir = $this->workDir.'cacheODT'.DIRECTORY_SEPARATOR;
			}else {
    ...
    function abrirOdt()
    {
      $nombreDir = array();
      $verificacion = '';
      $dirTmp = getcwd() . DIRECTORY_SEPARATOR;
      $creoDir = false;
      $existeDir = false;
	...
	{	$nombreDir = explode('.',$this->nombreOdt);
		...
		echo $error;
		$existeDir = is_dir($this->cacheDir.DIRECTORY_SEPARATOR. $nombreDir[0]);
		//echo $existeDir ."**** <hr>";
		if (!$existeDir)
		{
    ...
    function cargarContenido(){
		...
		if ($esDir){
          ...
          if ($this->fp3) {
            ...
            if(is_file($this->cabecerasXml2) &&  filesize($this->cabecerasXml2) > 0) {
		    $this->fp2 = fopen($this->cabecerasXml2 ,'r+');
                  @$this->contenidoCabeceras2 = fread ($this->fp2, filesize($this->cabecerasXml2));
                  fclose( $this->fp2 ) ;
            }
            if(is_file($this->cabecerasXml3) && filesize($this->cabecerasXml3) > 0)  {
...
if($esArchivoStyles)
{
	...
	$intBodega = strpos($this->workDir, DIRECTORY_SEPARATOR."bodega");
	if ( $intBodega === false )
	{
		$workDIR = str_replace(DIRECTORY_SEPARATOR."tmp".DIRECTORY_SEPARATOR."workDir", "",$this->workDir);
		$archivoTmp = str_replace(".".DIRECTORY_SEPARATOR."bodega", "",$archivoTmp);
		$archivoFinal = str_replace(".".DIRECTORY_SEPARATOR."bodega", "",$archivoFinal);
	}else
	{
		$workDIR = str_replace(DIRECTORY_SEPARATOR."bodega".DIRECTORY_SEPARATOR."tmp".DIRECTORY_SEPARATOR."workDir", "",$this->workDir);
		$archivoTmp = str_replace(".".DIRECTORY_SEPARATOR."bodega", DIRECTORY_SEPARATOR."bodega",$archivoTmp);
		$archivoFinal = str_replace(".".DIRECTORY_SEPARATOR."bodega", DIRECTORY_SEPARATOR."bodega",$archivoFinal);
	}
	if ($vieneTemp != null)
	{
		...
		if ($tipoUnitario == '1'){	
			...
			$cmd_cp = "cp " . $this->workDir.$nombreZip[0].".docx ".str_replace(DIRECTORY_SEPARATOR."tmp".DIRECTORY_SEPARATOR."workDir", "",$this->workDir).str_replace(".".DIRECTORY_SEPARATOR,"", $archivoFinal);
			switch ($this->SO)
			{	case 'W':
				{
				...
				default: 
					...
					}else{	$cmd_cp = "cp " . $this->workDir . $nombreZip[0] . ".docx " . $workDIR .  str_replace('/',DIRECTORY_SEPARATOR,$archivoFinal);
						switch ($this->SO)
						{	case 'W':
