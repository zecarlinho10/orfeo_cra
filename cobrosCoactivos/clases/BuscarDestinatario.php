...
/************************/
    ...
    {
        $isqlCodigo = "SELECT * FROM FLDOC.BODEGA_EMPRESAS
                       WHERE NOMBRE_DE_LA_EMPRESA LIKE '%" . strtoupper($nomb) ."%' OR NIT_DE_LA_EMPRESA LIKE '%" . $nomb ."%' ORDER BY NOMBRE_DE_LA_EMPRESA";

        $rs = $this->db->conn->SelectLimit($isqlCodigo, $cantidad, - 1);
        return $rs;
    }

    public function buscarFuncionarios($nombre,$catidad=30){
        $isqlCodigo = "SELECT ID, USUA_NOMB, USUA_LOGIN, DEPE_CODI, USUA_CODI
                        FROM USUARIO
                        WHERE USUA_NOMB LIKE '%".$nombre."%' AND 
                        USUA_ESTA=1";
        $rs = $this->db->conn->SelectLimit($isqlCodigo, $cantidad, - 1);
        return $rs;
    }
    ...
    public function guardarCiu($destinatario)
    {
        ...
        $insertSQL = $this->db->conn->Replace("SGD_CIU_CIUDADANO", $record, array(
            'SGD_CIU_CODIGO'
        ), $autoquote = true);
