...
class jhrtf {
    ...
    function __construct($archivo_insumo,$ruta_raiz,$arcPDF,&$db){
    ...
    function combinar_csv($dependencia,$codusuario,$usua_doc,$usua_nomb,$depe_codi_territorial,$codiTRD,$tipoRadf){
		...
		//$this->pdf->ezImage("$encaenti",0,500,'none','left');
        ...
        $ruta = BODEGA . "/masiva/".$this->arcCSV;
        clearstatcache();
		...
		$rad->esta_fisico = 0;
                ...
                /** 2023-06-09 cricaurte, la depedencia 10 (Direccion Ejecutiva)
		    con tipo de radicado 1 (salida) , por validar variables 
		    que llegan sin valor
		**/
		// $nurad = $rad->newRadicado($tipoRadf, $dependencia);    
                   $nurad = $rad->newRadicado($tipoRadf, 10);
            ...
            $this->resulComb = $data;
            $fileExito = BODEGA . "/masiva/$archInsumo.ok";
    ...
    function cargar_csv(){
        ...
        $fc = file_get_contents(BODEGA . "/masiva/" . $this->arcCSV);
        ...
        if ($fc)
        {
            ...
            echo "<BR> No hay un archivo csv llamado *". BODEGA . "/masiva/" . $this->arcCSV."*";
        }
    }	
    ...
    function cargarInsumo(){
        $fp=fopen(BODEGA . "/masiva/$this->archivo_insumo",'r');
        $i=0;
        while (!feof($fp))
        {	$i++;
