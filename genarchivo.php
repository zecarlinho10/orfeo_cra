...
//////////////////////
//$rad->raAsun = "Este es un asunto de test";
//////////////
            $rad->radiTipoDeri = 0;
            $rad->radiCuentai = "''";
...
$fp = fopen(BODEGA."/masiva/$archInsumo", 'w+');
...
fputs($fp, "archivoInicial=".BODEGA."$linkArchSimple" . "\n");
fputs($fp, "archivoFinal=".BODEGA."$archivoFinal" . "\n");
...
for ($i_count = 0; !empty($camposSanc) &&  $i_count < count($camposSanc); $i_count++) {
    fputs($fp, trim($camposSanc[$i_count]) . "=" . trim($datosSanc[$i_count]) . "\n");
}

for ($i_count = 0;!empty($campos) &&  $i_count < count($campos); $i_count++) {
...
if ($ext == "ODT" || $ext == "odt") {
    ...
    if (file_exists(BODEGA."/masiva/$archInsumo")) {
        $contenidoCSV = file(BODEGA."/masiva/$archInsumo");
    } else {
    ...
    $archivoACargar = BODEGA.str_replace(array('../','./','//'),array('',"",""), $linkarchivo);
...
if (isset($_REQUEST['clave']) && isset($_SESSION["usua_perm_firma"])){
    $archPdfC= str_replace("./.",BODEGA,$linkarchivo);
    ...
    $commandToPDF='soffice --headless --convert-to pdf '.$archPdfC;
...
if ($_SESSION['apiFirmaDigital']!='certicamara'){

    $commandFirmado='java  -jar '.BODEGA.'/include/JSignPdf-1.5.1/JSignPdf.jar '.str_replace('odt','pdf',$nombreArchivo).' -kst PKCS12  -ksf '.BODEGA.'/firmas/'.$usua_doc.'.p12   -ksp '.$clave.' --font-size 7    -r \'Firmado al Radicar en OrfeoGPL\'  -V -v   --img-path '.$_SESSION['RUTA_ABSOLUTA'].'/imagenes/gnu.gif --render-mode  GRAPHIC_AND_DESCRIPTION -llx 0 -lly 0 -urx 550 -ury 27';
...
$fileToSignPath = str_replace(".odt",".pdf",BODEGA."/tmp/workDir/$nombreArchivo");
//ruta para guardar el archivo firmado
$fileSignedPath = BODEGA."/$pathFinal";
//ruta del certificado de firma
//$signP12Path = "$root/resources/certificate/cra.p12";
$signP12Path = BODEGA.'/firmas/'.$usua_doc.'.p12';
...
//ruta del certificado de firma
//$signP12Path = $_SESSION['RUTA_ABSOLUTA'].'/bodega/firmas/'.$usua_doc.'.p12';
//password del certificado de firma
...
try {
    ...
    $isql="update anexos set ANEX_NOMB_ARCHIVO='$nombreArchivoFinal', ANEX_TIPO=7, ANEX_ESTADO=3, SGD_FECH_IMPRES= (SYSDATE+0), ANEX_FECH_ENVIO=(SYSDATE+0), SGD_DEVE_FECH = NULL, SGD_DEVE_CODIGO=NULL where RADI_NUME_SALIDA=$rad_salida";
...
$linkArchivo=(!empty($linkArchivo))?$linkArchivo:$anexo;
    ...
    define ('CACHE', WORKDIR . '/cacheODT/');
    //Se abre archivo de insumo para lectura de los datos
    $fp = fopen(BODEGA."/masiva/$archInsumo", 'r');
...
if ($_SESSION['apiFirmaDigital']!='certicamara'){
    $commandFirmado='java  -jar '.BODEGA.'/include/JSignPdf-1.5.1/JSignPdf.jar '.str_replace('docx','pdf',$nombreArchivo).' -kst PKCS12  -ksf '.BODEGA.'/firmas/'.$usua_doc.'.p12   -ksp '.$clave.' --font-size 7    -r \'Firmado al Radicar en OrfeoGPL\'  -V -v   --img-path '.BODEGA.'/imagenes/gnu.gif --render-mode  GRAPHIC_AND_DESCRIPTION -llx 0 -lly 0 -urx 550 -ury 27';
    if (exec($commandFirmado)=="INFO  Finished: Creating of signature failed."){
			unset($answer);
			$answer=array();
                    saveMessage('error',"Clave de firma digital erronea");
    ...
    rename(str_replace(".docx","_signed.pdf",$nombreArchivo), BODEGA."$pathFinal" );
}else  {
include ("include/apiCerticamara/PdfSign.php");
//ruta archivo a firmar
$fileToSignPath = str_replace(".docx",".pdf",BODEGA."/tmp/workDir/$nombreArchivo");
//ruta para guardar el archivo firmado
$fileSignedPath = BODEGA."/$pathFinal";
//ruta del certificado de firma
//$signP12Path = "$root/resources/certificate/cra.p12";
...
$link         = BODEGA. "/" . $linkarchivo_grabar;
$tam          = @filesize($link);
$linkFuente   = str_replace("d.", ".", $linkarchivo_grabar);
$linkF        = BODEGA. "/" . $linkFuente;
