...
$encabezado = "?opcionExp=$opcionExp&numeroExpediente=$numeroExpediente&dependencia=$dependencia&krd=$krd&nurad=$nurad&coddepe=$coddepe&codusua=$codusua&depende=$depende&ent=$ent&tdoc=$tdoc&codiTRDModi=$codiTRDModi&codiTRDEli=$codiTRDEli&codserie=$codserie&tsub=$tsub&ind_ProcAnex=$ind_ProcAnex";
...
if( isset( $_POST['expSeleccionados'] ) && $_POST['expSeleccionados'] != "" )
{
	...
	{
	  //require_once(realpath(dirname(__FILE__))."/../conf/configArchivo.php");
	  //require_once(realpath(dirname(__FILE__)) ."/../include/NotificaMail.php");
	  //$notificacion = new NotificaMail();
	  //$notificacion->notificar("Se han excluido el radicado". $_GET['nurad']." del/los Expedientes".$_POST['expSeleccionados'],$buzonArchivoNotifica,"Excluir radicado de Expediente");
        foreach( $arrExpSeleccionados as $clave => $numExpediente )
        {
            ...
            if( $resultadoExp == 1 )
            {
				...
				// David Amaya Beltran - Historico excluye radicado 
				$observa = "*Se excluye del expediente -> * ".$numExpediente;
				$codiRegH[] =$_GET['nurad'];
              ...
              <div class="table-responsive">
                <table class="table table-bordered table-striped">
                    ...
                    <td height="30" colspan="2" class="listado2">
                        <center>
                          <input class="botones btn btn-xs btn-primary" type="button" name="btnExcluir" id="btnExcluir" Value="EXCLUIR" onClick="excluirExpediente();">
                      </center>
                      </td>
                    <td width="50%" height="30" colspan="2" class="listado2"><center>
                      <input class="botones btn btn-xs btn-primary" type="button" name="Cancelar" id="Cancelar" value="CANCELAR" onClick="window.opener.$.fn.cargarPagina('expediente/lista_expedientes.php','tabs-a'); window.close();"></center>  </td>
                    </tr>
                </table>
             </div>
<?php
              ...
              <div class="table-responsive">
                <table class="table table-bordered table-striped">
                    <tr align="center">
                      <td width="33%" height="25" class="listado2" align="center">
                      <center>
                        <input name="btnConfirmar" type="button" onClick="confirmaExcluir();" class="botones_funcion btn btn-xs btn-primary" value="Confirmar">
                      </center>
                      </td>
                    <td width="33%" class="listado2" height="25">
                    <center><input name="cerrar" type="button" class="botones_funcion btn btn-xs btn-primary" id="envia22" onClick="opener.regresar(); window.close();" value=" Cerrar "></center></TD>
                    </tr>
                  </table>
            </div>
<?php	
