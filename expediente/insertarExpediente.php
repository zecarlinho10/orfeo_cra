...
if( $funExpediente == "INSERT_EXP" ){
    ...
    foreach ( $arrExpedientes as $line_num => $line){
        ...
        if( $resultadoExp == 1 )
        {
            ...
            $tipoTx = 53;
			$inc_exp=$_REQUEST['numeroExpediente'];
            $Historico = new Historico( $db );
            $Historico->insertarHistoricoExp($inc_exp, $radicados, $dependencia, $codusuario, $observa, $tipoTx, 0 );
				
				//Agregar al historico incluir y excluir
				
				$observa = "*Se incluye en el expediente -> * ".$inc_exp;
				$codiRegH[] =$_REQUEST['nurad'];
  		  		$Historico1 = new Historico($db);		  
  		  		$Historico1->insertarHistorico($codiRegH, $dependencia, $codusuario, $dependencia, $codusuario, $observa,$tipoTx); 
...
<form method="post" action="<?=$_SERVER['PHP_SELF']?>?nurad=<?=$nurad?>" name="insExpBus">
                <table class="table table-bordered table-striped">
                  <tr align="center">
                    ...
                    <td  valign="middle">
                      <br><input type="text" name="criterio" id="criterio" size="30" value="<?=trim($_POST['criterio'])?>" class="tex_area">
                    </td>
                  </tr>
                </table>
              </div>
...
if($btnBuscaExp){
	$criterio = strtoupper(trim($_POST['criterio']));
	if(!$criterio)
	{
		$criterio="_nada_";
    //echo $criterio;
...
<?
      // Consulta si existe el expediente Y pertenece a la dependencia.

      if ( $expediente->existeExpediente( $_POST['numeroExpediente'] ) !== 0 && $expediente->confirmaDependencia( $_POST['numeroExpediente'],$krd) !=0) {
          $numeroExpediente = $_POST['numeroExpediente'];
          $sql_cexp = "SELECT SGD_CERRADO FROM SGD_SEXP_SECEXPEDIENTES where sgd_exp_numero = '$numeroExpediente'";
          $rs_Cexp = $db->conn->query($sql_cexp);
          $exp_cerrado = intval($rs_Cexp->fields['SGD_CERRADO']);
                      ...
                      <div class="table-responsive">
                        <table class="table table-bordered table-striped">
                          <tr align="center">
                            <td width="33%" height="25"  align="center">
                              <center >
                                NO SE PUEDE INCLUIR EL RADICADO A ESTE EXPEDIENTE:
                              </center>
                              <B>
                                <center class="style1"><b><?php print $numeroExpediente; ?></b></center>
                              </B>
                              <div align="justify"><br>
                                <strong><b>Advertencia:</b>Este expediente esta Cerrado.
                              </strong>
                             </div>
                            </td>
                          </tr>
                      ...
                      <div class="table-responsive">
                        <table class="table table-bordered table-striped">
                          <tr align="center">
                            <td width="33%" height="25"  align="center">
                            <center>
                              <input name="btnConfirmar" type="button" onClick="confirmaIncluir();" class="botones_funcion btn btn-xs btn-primary" value="Confirmar">
                            </center>
                            </td>
                          <td width="33%"  height="25">
                          <center><input name="cerrar" type="button" class="botones_funcion btn btn-xs btn-primary" id="envia22" onClick="window.opener.$.fn.cargarPagina('expediente/lista_expedientes.php','tabs-a'); window.close();" value=" Cerrar "></center></TD>
                          </tr>
                        </table>
                      </div>
                      <?php 
        ...
        <?php } 
        // 4/2/2019 Carlos Ricaurte: invoca confirmaDependencia() para validar si el usuario pertenece a la dependencia del expediente
        else if (  $_POST['numeroExpediente'] != "" &&  $expediente->confirmaDependencia( $_POST['numeroExpediente'] , $krd ) == 0 ){ ?>
          <script language="JavaScript">
            alert( "Error. El Expediente No pertenece a esta dependencia." );
            document.getElementById( 'numeroExpediente' ).focus();
          </script>
        <?php } 
