...
*/
  ...
  $cambioKrd      = $_SESSION["cambioKrd"];
  $cambioDepeCodi = $_SESSION["cambioDepeCodi"];
  $cambioUsuaCodi = $_SESSION["cambioUsuaCodi"];
  
if($cambioKrd){
  $_SESSION["krd"] = $cambioKrd;
  $_SESSION["dependencia"] = $cambioDepeCodi;
  $_SESSION["codusuario"] = $cambioUsuaCodi;
  ...
  foreach ($tpNumRad as $key => $valueTp){
    ...
    $enlace9   = "href=\"radicacion/chequear.php?$phpsession&fechah=$fechah&primera=1&ent=$valueTp&depende=$dependencia\"";
 ...
 if ($_SESSION["usua_perm_owncloud"]>=1) {
       $enlace13 = "href=\"uploadFiles/owncloud/orfeoCloud.php?$phpsession&fechah=$fechah&primera=1&ent=2&depende=$dependencia\"";
       $linkrad .= "<li><a $enlace13 target=\"mainFrame\">Cargar Archivos Owncloud</a></li>";
     }

  if ($_SESSION["perm_radi"]>=1){
      $enlace11 = "href=\"uploadFiles/uploadFileRadicado.php?$phpsession&fechah=$fechah&primera=1&ent=2&depende=$dependencia\"";
      $linkrad .= "<li><a $enlace11 target=\"mainFrame\">Asociar Imagenes</a></li>";
  }

  if ($_SESSION["usuaPermRadEmail"]==1) {
    $enlace12 = "href=\"email/index.php?$phpsession&fechah=$fechah&primera=1&ent=2&depende=$dependencia\"";
...
<html lang="es">
    <head>
       ...
       si no existe se toma el favicon por defecto-->
        <link rel="shortcut icon" href="./img/<?=(file_exists("./img/$entidad.favicon.png"))?$entidad.".":""?>favicon
       .png" onClick="this.reload();">
        <!-- Bootstrap core CSS -->
  ...
  <!--Si existe un logoEntidad especifico para la entidad su nombre debe de ser asi <entidad>.favicon.png, si no existe se toma el favicon por defecto-->
              <a class="navbar-brand" align="center" onclick="recapagi()" href="#" alt="<?=$entidad_largo?>" title="<?=$entidad_largo?>"><img border=0 src="<?=$ruta_raiz?>/img/<?=(file_exists("./img/$entidad.logoEntidad.png"))?$entidad.".":""?>logoEntidad.png" width="25" height="22"></a>
            </div>
            ...
            <div class="collapse navbar-collapse">
                ...
                <ul class="nav navbar-nav">
				  ...
				  <?php if ($usua_doc == '1118806537'){?><li> <a href="instalacion/recuperar_radicado.php" target='mainFrame' > Recuperar Radicados </a></li><?php } ?>
...
<?php
  ...
  <a href="./Administracion/tbasicas/adm_dependencias.php?<?=$sendSession?>"
		 ...
		 <?php
                       // Busca si el usuario tiene dos perfiles en la entidad con igual correo electronico
                       $usua_email = $_SESSION["usua_email"];
		       $isql = "select u.usua_nomb, u.usua_login, u.depe_codi, d.depe_codi, d.depe_nomb
                                 from usuario u, dependencia d 
                                 where u.depe_codi=d.depe_codi and u.usua_email='$usua_email'";
		       // $db->conn->debug = true;
                       $rs = $db->conn->query($isql);
                      ?>
		   ...
		   <?php
		     while(!$rs->EOF){
		       ...
		       $usuaCodi = $rs->fields["USUA_CODI"];	
                   ?>
		      <li><a href="index_frames.php?<?="fechah=$fechah&cambioKrd=$usuaLogin&cambioDepeCodi=$depeCodi&cambioUsuaCodi=$usuaCodi&info=false"?>" ><i class="fa fa-user"></i> <?=$depeCodi." - $depeNomb ($usuaLogin) " ?> </a></li>
                    <?
		      $rs->MoveNext();
		     }
		      ?>
