...
$recaptcha_secret = '6LfZaAEmAAAAAD_wJCpqlUKn9YdEHR2jMXmi8t4G'; 
...
if ($errorFormulario==0) {	if (! empty($_FILES) || ! empty($_POST["adjuntosSubidos"])) {
    ...
    $numeroRadicado = $radicacionAtencion->radicarCCU($db, $destinatario, $documento, $usr, $tipoRad, $uploader, $_POST, "768.odt");

    $atencion["radicado"] = $numeroRadicado["radicado"];

    if (! empty($_FILES) || ! empty($_POST["adjuntosSubidos"])) {
    	//echo "entra segundo IF";
    	$uploader->moverArchivoCarpetaBodega($numeroRadicado["radicado"],"321");
    	$uploader->moverArchivoCarpetaBodegaYaSubidos($numeroRadicado["radicado"],"321");
    }
