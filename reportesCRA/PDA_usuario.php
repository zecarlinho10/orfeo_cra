...
<form action="PDA_usuario.php?<?=$params?>" method="post" enctype="multipart/form-data" name="formSeleccion" id="formSeleccion" >
...
//$db->conn->debug = true;
 ...
 <?
$sqlus= "SELECT 'TODOS LOS USUARIOS' as USUA_NOMB, 0 AS USUA_CODI FROM USUARIO
                 UNION  SELECT USUA_NOMB, USUA_CODI AS USUA_CODI FROM USUARIO
...
$rssr = $db->query($sqlsr);
$fechare= $rssr->fields['FECHAORI'];

$sqlfechv="select fech_vcmto from radicado where radi_nume_radi= '$radicadoin'";
			//$db->conn->debug = true;
$rsfechv = $db->query($sqlfechv);
$rgfechv = $rsfechv->fields['FECH_VCMTO'];
//echo $rsfechv;
...
?>
        <tr>
                ...
                <td class="listado2" align="center"><?=$rgfechv?></td>
