...
<tr>
    <td align="center" width="30%" class="titulos2">Medio de recepción</td>
    <td class="listado2">
        <?
           $sql = " SELECT 'Todos los medios' as MREC_DESC, 0 AS MREC_CODI FROM MEDIO_RECEPCION
  UNION  SELECT 'CCU' as MREC_DESC, 101 AS MREC_CODI FROM MEDIO_RECEPCION
  UNION  SELECT 'PQR WEB' as MREC_DESC, 102 AS MREC_CODI FROM MEDIO_RECEPCION
  UNION  SELECT MREC_DESC, MREC_CODI AS DEPE_CODI FROM MEDIO_RECEPCION";
            $rsMedio = $db->conn->Execute($sql);
            if(!$s_DEPE_CODI) $s_DEPE_CODI= 0;
            print $rsMedio->GetMenu2("medio","$medio",false, false, 0," class='select'");
...
if($medio<>0){
    ...
    if($medio==102){
        $ldpdd=", case when LDPDD = 1 then 'SI' else 'NO' end as LEY_DPDD ";
        $where.= " AND A.MREC_CODI=3 AND A.RA_ASUN NOT LIKE '%Asistente para generación de Contratos de Condiciones Uniformes (CCU)%' ";
    }
    else if($medio==101){
        $where.= " AND A.MREC_CODI=3 AND A.RA_ASUN LIKE '%Asistente para generación de Contratos de Condiciones Uniformes (CCU)%' ";
    }
    else{
        $where.= " AND A.MREC_CODI=".$medio;
    }
}      
            ...
            $isql = "SELECT distinct A.RADI_NUME_RADI AS ENTRADA, A.RA_ASUN AS ASUNTO, to_char(A.radi_fech_radi,'yyyy/mm/dd hh24:mi:ss') AS FECHAS, 
                ...
                when SGD_DOC_FUN IS NOT NULL 
                    then 'FUNCIONARIO' 
                     ...
                     MR.MREC_DESC AS MEDIO, C.TIPO_PQR AS PQR
                     FROM RADICADO A
...
//cjcc, se realiza cambio de la ruta, la que esta se encuentra en otra ruta del archivo, igual 

require_once (realpath(dirname(__FILE__) . "/../")."/include/class/adodb/adodb-pagination-cb.php");
