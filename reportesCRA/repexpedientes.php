...
<table width="57%" border="0" cellspacing="5" cellpadding="0" align="center" class='table table-bordered table-striped mart-form'>
   ...
   $sql = "	SELECT 'Todas las dependencias' as DEPE_NOMB, 0 AS DEPE_CODI FROM DEPENDENCIA
            ...
            UNION  SELECT D.DEPE_NOMB, D.DEPE_CODI AS DEPE_CODI FROM DEPENDENCIA D, SGD_SEXP_SECEXPEDIENTES S
			WHERE D.DEPE_CODI NOT IN (900,905,999,910,1,321,210,77) AND S.DEPE_CODI = D.DEPE_CODI
...
$where=null;
			...
			if ($dep==3)
			{
			  $where=" AND s.depe_codi in (301,310,320,341,350,20) ";	
			}
    ...
    $isql= "select distinct(s.sgd_exp_numero) AS EXP,
                    ...
                    nvl((select sum(radi_nume_hoja) as hojas from radicado r, sgd_exp_expediente x where 
                    r.radi_nume_radi=x.radi_nume_radi and x.sgd_exp_numero=s.sgd_exp_numero
                    and x.sgd_exp_numero=s.sgd_exp_numero ),0) as FOLIOS,  s.sgd_cerrado,
            ...
            from sgd_sexp_secexpedientes s
            		--INNER JOIN SGD_EXP_EXPEDIENTE E ON S.sgd_exp_numero = E.sgd_exp_numero,
                       LEFT OUTER JOIN SGD_EXP_EXPEDIENTE E ON S.sgd_exp_numero = E.sgd_exp_numero,
            		dependencia d
                    where  
                    s.depe_codi not in (900,905,910,999,77) and
