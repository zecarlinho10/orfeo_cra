...
<table width="57%" border="0" cellspacing="5" cellpadding="0" align="center" class='table table-bordered table-striped mart-form'>
	...
	<tr align="center" colspan="2">
		...
		<td width="69%" height="30" class='listado2' align="left">
			<input name="serie" id="serie" type="input" size="50" class="tex_area" value="<?php echo $serie?>" />
		</td>
		</tr>	
	...
	<tr align="center" colspan="2">
		...
		<td width="69%" height="30" class='listado2' align="left">
			<input name="tipo" id="tipo" type="input" size="50" class="tex_area" value="<?php echo $tipo?>" />
		</td>
		</tr>
		...
		<td width="69%" height="30" class='listado2' align="left">
		 ...
		 <tr>
      <td class="titulos2">Desde Fecha (dd/mm/yyyy)</td>
      <td class="listado5">
  ...
  ?>
    ...
    <tr>
      <td class="titulos2">Hasta Fecha (dd/mm/yyyy)</td>
      <td class="listado5">
...
if(!empty($_POST['Consultar'])&& ($_POST['Consultar']=="Consultar")){
			...
			$ps_desde_RADI_FECH_RADI = mktime(0,0,0,$_POST[ 's_desde_dia'],$_POST["s_desde_mes"],$_POST["s_desde_ano"]);
 			$ps_hasta_RADI_FECH_RADI = mktime(23,59,53,$_POST["s_hasta_dia"],$_POST["s_hasta_mes"],$_POST["s_hasta_ano"]);
			...
			$where = (empty($where) && ($where==null))?" WHERE RADI_FECH_RADI>=".$db->conn->DBTimeStamp($ps_desde_RADI_FECH_RADI):$where." AND  RADI_FECH_RADI>=".$db->conn->DBTimeStamp($ps_desde_RADI_FECH_RADI);
 			...
 			$order=1; 
 			$titulos=array("1#RADICADO","2#FECHA RADICACION","3#NOMBRE","4#TIPO","5#SERIE","6#CARPETA","7#CAJA","8#FECHA INICIAL","9#FECHA FINAL");
