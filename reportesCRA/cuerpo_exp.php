...
$fechf=fnc_date_calc($fechafi,1);
	  ...
	  $dependencia_busq1= " and d.sgd_exp_estado!=2 and d.depe_codi like '$dep_sel'  and (upper(d.sgd_exp_numero) like '%$buscar_exp%' and upper(d.RADI_NUME_RADI) like '%$buscar_rad%')";
	  ...
	  $dependencia_busq1.=" and d.sgd_exp_fech >= '2007-03-01' ";
if($buscar_eti!=""){
$etim=strtoupper ($buscar_eti);
$etiM=strtolower ($buscar_eti); 
$table=", sgd_sexp_secexpedientes sexp";
$dependencia_busq1.=" and (sexp.sgd_sexp_parexp5 like '%$etim%' 
or sexp.sgd_sexp_parexp5 like '%$etiM%' or sexp.sgd_sexp_parexp5 like '%$buscar_eti%' or sexp.sgd_sexp_parexp1 like '%$etim%' or sexp.sgd_sexp_parexp1 like '%$buscar_eti%' or sexp.sgd_sexp_parexp1 like '%$etiM%' or sexp.sgd_sexp_parexp2 like '%$etim%' or sexp.sgd_sexp_parexp2 like '%$buscar_eti%' or sexp.sgd_sexp_parexp2 like '%$etiM%' or sexp.sgd_sexp_parexp3 like '%$etim%' or sexp.sgd_sexp_parexp3 like '%$buscar_eti%' or sexp.sgd_sexp_parexp3 like '%$etiM%' or sexp.sgd_sexp_parexp4 like '%$etim%' or sexp.sgd_sexp_parexp4 like '%$buscar_eti%' or sexp.sgd_sexp_parexp4 like '%$etiM%') and sexp.sgd_exp_numero=d.sgd_exp_numero and sexp.depe_codi != 900 and sexp.depe_codi !=905 ";

}

$fechah=date("dmy") . "_". date("h_m_s");
$encabezado = session_name()."=".session_id()."&buscar_exp=$buscar_exp&buscar_rad=$buscar_rad&buscar_eti=$buscar_eti&krd=$krd&tipo_archivo=$tipo_archivo&nomcarpeta=$nomcarpeta&fechai=$fechai&fechafi=$fechafi";
$tbbordes = "#CEDFC6";
$tbfondo = "#FFFFCC";
if(!$orno){$orno=1;}
...
if($tipo_archivo==2){$img7=" <img src='../iconos/flechanoleidos.gif' border=0 alt='$data'> ";}
  ...
  $datosaenviar = "buscar_exp=$buscar_exp&buscar_rad=$buscar_rad&buscar_eti=$buscar_eti&buscar_esp=$buscar_esp&fechaf=$fechaf&tipo_carp=$tipo_carp&ascdesc=$ascdesc&orno=$orno&fechai=$fechai&fechafi=$fechafi";
  $encabezado = session_name()."=".session_id()."&buscar_exp=$buscar_exp&buscar_rad=$buscar_rad&buscar_eti=$buscar_eti&krd=$krd&fechah=$fechah&ascdesc=$ascdesc&dep_sel=$dep_sel&tipo_archivo=$tipo_archivo&nomcarpeta=$nomcarpeta&fechai=$fechai&fechafi=$fechafi&sel=$sel&Buscar=$Buscar&orno=";
  $encabezado2 = session_name()."=".session_id()."&buscar_exp=$buscar_exp&buscar_rad=$buscar_rad&buscar_eti=$buscar_eti&krd=$krd&fechah=$fechah&ascdesc=$ascdesc&dep_sel=$dep_sel&tipo_archivo=$tipo_archivo&nomcarpeta=$nomcarpeta&fechai=$fechai&fechafi=$fechafi&sel=$sel&Buscar=$Buscar&orno=";
    $fechah=date("dmy") . "_". date("h_m_s");
...
?>
<table  WIDTH='100%' class='table table-bordered table-striped mart-form' valign='top' cellspacing="0">
<tr class="tablas">
...
&nbsp;Etiqueta &nbsp&nbsp&nbsp&nbsp;
<input type=text name=buscar_eti value='<?=$buscar_eti?>' class="tex_area"><br>
...
<table><tr><td></td></tr></table>
 <table width='100%' class='table table-bordered table-striped mart-form'>
  <tr>
  <td  align='left' height="40" class=titulos5 >Listar Por:
	...
	?>
	Por Archivar</span></a>
    <? if ($tipo_archivo==1)?> <span class="no_leidos" style="color:##00CC00" >
	Archivados</span>
    <br>
	</td>
	</tr>
  ...
  <tr>
	<td class="grisCCCCCC">
		<table cellspacing="3"  WIDTH=100% class='table table-bordered table-striped mart-form' align='center' >
		<tr  class="titulos5">
	<td  align="center">
...
<td width='10%' align="center"> <a href='cuerpo_exp.php?<?=$encabezado2 ?>2&ordcambio=1' class='textoOpcion' alt='Seleccione una busqueda'>
	...
	<?
	if( $tipo_archivo == 0 or $tipo_archivo==1){
?>
	<td  width='15%' align="center"> Etiqueta</td>
	<td  width='15%' align="center"> ESP </td>
	<td  width='15%' align="center"> DEPARTAMENTO </td>
	<td  width='15%' align="center"> OTRAS EMPRESAS </td>
...
// Comienza el ciclo para mostrar los documentos de la carpeta predeterminada.
   ...
   while(!$rs->EOF)
	...
	//$db->conn->debug=true;
	$rexp=$db->query($tm7);
	$etiqueta=$rexp->fields['SGD_SEXP_PAREXP5'];
	$esp=$rexp->fields['SGD_SEXP_PAREXP1'];
	$dpto=$rexp->fields['SGD_SEXP_PAREXP2'];
	$oem=$rexp->fields['SGD_SEXP_PAREXP3'];
    /*
     *  Modificado: 22-Septiembre-2006 Supersolidaria
...
<td class='<?=$leido ?>' width="20%"> <span class='<?php print $class; ?>'><?=$etiqueta?> </span>
</td>
<td class='<?=$leido ?>' width="20%"> <span class='<?php print $class; ?>'><?=$esp?> </span>
</td>
<td class='<?=$leido ?>' width="20%"> <span class='<?php print $class; ?>'><?=$dpto?> </span>
</td>
<td class='<?=$leido ?>' width="20%"> <span class='<?php print $class; ?>'><?=$oem?> </span>
</td>
...
<table border=0 cellspace=2 cellpad=2 WIDTH=100% class='table table-bordered table-striped mart-form' align='center'>
<tr align="center">
	<td> <?
