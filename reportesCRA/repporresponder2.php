...
if (! empty($fvcmto) && $dependencia!=999 ) {
    $dias = $calendario->calcularDias($fvcmto, date('Y-m-d'));
    $diaSemana = strtotime($fvcmto);
    $diasHoy = time();
    $diasRestantes= ceil(($diaSemana-$diasHoy ) / (24 * 60 * 60));
   
} else if(empty($fvcmto)){
...
?>
        <tr>
                ...
                <td class="listado2" align="center"><?=intval($diasRestantes)?></td>
