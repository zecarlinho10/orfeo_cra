<?
 /* Programa que despliega Radicados de Salidas y el medio de envío en un periodo determinado
 * @author  carlos ricaurte
 * @mail    cricaurte@cra.gov.co
 * @author  modify by Carlos Ricaurte
 * @mail   carlinhoricaurte@gmail.com    
 * @version     1.0
 */
$ruta_raiz = "../";
session_start();
...
<form action="rep36-3.php?<?=$params?>" method="post" enctype="multipart/form-data" name="formRadi" id="formRadi" >
...
function pintarResultados($fila,$i,$n){
		...
		switch($n){
			case 0:
				$salida=$fila['RADI_NUME_SAL'];
 				break;
 			case 1:
				$salida=$fila['SGD_RENV_FECH'];
				break;	
			case 2:
			...
			case 3:
				...
				break;		
			case 4:
				$salida=$fila['SGD_RENV_DESTINO'];
				break;	
			case 5:
				$salida=$fila['SGD_RENV_NOMBRE'];
				break;
			case 6:
				$salida=$fila['SGD_FENV_DESCRIP'];
				break;
...
$where=null;
			...
			$where=(!empty($_POST['dep']) && ($_POST['dep'])!="")?"AND e.depe_codi = ".$_POST['dep']:"";		
			
			
if ($dep==2)
{
  $where=" AND e.depe_codi in (401,410,420,430) ";	
}	
if ($dep==3)
{
  $where=" AND e.depe_codi in (301,310,320,341,350) ";	
}	
if ($dep==4)
{
  $where=" AND e.depe_codi in (211,212,213,214,215) ";	
}  		   					   			
 		
			$where.= " AND TRUNC (SGD_RENV_FECH) BETWEEN (".$db->conn->DBTimeStamp($fecha_ini).") AND (".$db->conn->DBTimeStamp($fecha_fin).")";
 		
 			
 			$order=1;  
			    	
		 
$titulos=array("SALIDA","FECHAS","DEPENDENCIA", "DEPARTAMENTO","MUNICIPIO","DIRECCION","TIPO ENTREGA");
			
			$isql = "SELECT RADI_NUME_SAL, SGD_RENV_FECH, DEPE_NOMB, SGD_RENV_DEPTO, SGD_RENV_DESTINO, SGD_RENV_NOMBRE, SGD_FENV_DESCRIP
					FROM SGD_RENV_REGENVIO E, SGD_FENV_FRMENVIO T, DEPENDENCIA D
					WHERE T.SGD_FENV_CODIGO=E.SGD_FENV_CODIGO AND D.DEPE_CODI=E.DEPE_CODI {$where} ORDER BY SGD_RENV_FECH";
...
error_reporting(0);
	...
	$campos_tabla = array("RADI_NUME_SAL","SGD_RENV_FECH","DEPE_NOMB", "SGD_RENV_DEPTO","SGD_RENV_DESTINO","SGD_RENV_NOMBRE","SGD_FENV_DESCRIP");
