...
/**
 *
 * Programa que d:wqespliega el formulario de consulta de ESP
...
// No olvida declarar variable que pasan por parametro POST
$dep = $_POST['dep'];
...
<form action="repradentrada.php?<?=$params?>" method="post"
			enctype="multipart/form-data" name="formSeleccion" id="formSeleccion">

			<table width="57%" border="0" cellspacing="5" cellpadding="0"
				align="center" class='table table-bordered table-striped mart-form'>
				<tr align="center">
					<td height="25" colspan="4" class='titulos2'>CONSULTA RADICADOS DE
						ENTRADA EN PROCESO <input name="accion" type="hidden" id="accion">
...
?>
				...
				<tr>
					<td align="center" width="30%" class="titulos2">Desde fecha
						(aaaa/mm/dd)</td>
	...
	dateAvailable.dateFormat="yyyy/MM/dd";
				...
				<tr>
					<td align="center" width="30%" class="titulos2">Hasta fecha
						(aaaa/mm/dd)</td>
					<td class="listado2"><script language="javascript">
	...
	dateAvailable2.dateFormat="yyyy/MM/dd";
				...
				<tr align="center">
					<td height="30" colspan="4" class='listado2'>
						...
						<center>
							<input name="Consultar" type="submit" class="botones"
								id="envia22" value="Consultar">&nbsp;&nbsp;
...
if (! empty($_POST['Consultar']) && ($_POST['Consultar'] == "Consultar")) {
    ...
    a.ANEX_BORRADO, c.carp_desc AS CARPETA, c.carp_codi,
    to_char(r.radi_fech_radi,'dd/mm/yyyy hh24:mi:ss') as FECHAE, 
    a.radi_nume_salida as RSALIDA, 
    to_char(a.anex_radi_fech,'dd/mm/yyyy hh24:mi:ss') as FECHAS,
    td.sgd_tpr_descrip AS TIPO, 
    r.ra_asun AS ASUNTO, 
    DR.sgd_dir_nomremdes REMITENTE,
    td.sgd_tpr_termino as termino,
    d.depe_nomb AS DEPE, 
    u.usua_nomb AS FUNCIO, 
...
<table border=2 align="center"
			class="table table-bordered table-striped mart-form" width="80%">
			<tr>
				...
				<td class="titulos2" align="center">CARPETA</td>
				...
				<td class="titulos2" align="center">DIAS RESTANTES HAB.</td>
				<!-- td class="titulos2" align="center">DIAS RESTANTES CAL.</td-->
			</tr>
<?php
    ...
    while (! $rs->EOF) {
        ...
        // Ajustado Nuevo Reporte por CARLOS RICAURTE 06/04/2021
      
        if (! empty($fvcmto)) {
            $dias = $calendario->restaFechasHabiles(date('Y-m-d'), $fvcmto);
        } else {
            $dias = $calendario->restaFechasHabiles(date('Y-m-d'), date_format($fechar,"Y-m-d"));
        }
